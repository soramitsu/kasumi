...
impl std::fmt::Debug for OpeningAdmission {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("OpeningAdmission")
...
impl crate::StorageAdmission for OpeningAdmission {
    ...
    fn reserve_workspace(
        ...
        if self.failed.load(Ordering::Acquire) {
            return Err(crate::AdmissionError::OwnerFailed);
        }
        ...
        let view = match self.outcome.get() {
            ...
            Some(FenceOutcome::Panicked(payload)) => match payload.try_lock() {
                ...
                Err(std::sync::TryLockError::Poisoned(error)) => {
                    FenceView::Panicked(error.into_inner())
                }
...
impl RetainedDatabaseOpening {
    ...
    // Split at the real ownership boundary. No callback or terminal work runs
    // before the bootstrap is installed; native fault tests arm failures here.
    fn begin_open(&mut self) -> bool {
        ...
        if matches!(self.opening, Observation::Panicked(_)) {
            // A backend/callback unwind can precede CheckedBackend's Result.
            // Keep that original payload before recording fence-hook effects.
    ...
    fn finish_bootstrap(&mut self) {
        ...
        let bootstrap = self
            .bootstrap
            .as_mut()
            .expect("bootstrap installed before body");
        ...
        self.opening_phase = DatabaseOpenPhase::BootstrapTerminal;
        self.phase = DatabaseOpenPhase::BootstrapDisposal;
        let database = self
            .database
            .as_ref()
            .expect("database installed before bootstrap");
        let report = bootstrap.dispose_settled(database);
        // A refused commit with successful rollback/disposal remains an opening
        // failure at its original terminal phase. A later close cannot erase it.
        if terminal_succeeded {
            self.opening_phase = DatabaseOpenPhase::BootstrapDisposal;
            if report.disposal_complete() {
                self.phase = DatabaseOpenPhase::Ready;
                self.opening_phase = DatabaseOpenPhase::Ready;
    ...
    fn open_body(&mut self) -> Result<(), DatabaseError> {
        ...
        let memory = self
            .memory
            .as_mut()
            .expect("memory installed before verification");
