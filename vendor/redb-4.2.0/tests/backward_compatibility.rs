...
fn test_helper<K: TestData + redb::Key + redb2_6::Key + 'static, V: TestData + 'static>() {
    {
        ...
        let db =
            redb::Database::open(tmpfile.path(), crate::admission_support::admission()).unwrap();
...
fn composite_of_user_type_created_by_redb2_6_still_opens() {
    ...
    let db = redb::Database::open(tmpfile.path(), crate::admission_support::admission()).unwrap();
