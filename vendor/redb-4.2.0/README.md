...
This Kasumi prerequisite fork requires an installed physical admission owner.
See [patch contract and provenance](KASUMI_PATCH.md); it is not yet activated in
Kasumi's production dependency graph.
...
fn example(admission: std::sync::Arc<dyn redb::StorageAdmission>) -> Result<(), Error> {
    let db = Database::create("my_db.redb", admission)?;
...
This is an unactivated Kasumi first-release prerequisite. It does not provide
backwards-compatible constructor or durability APIs, or a Kasumi migration path.
See the patch contract and retained validation evidence before integration.
