...
impl<'de> Visitor<'de> for KeyClassifier {
    ...
    where
        E: de::Error,
    {
        // Every JSON string key is user data, even if it spells a private token.
    ...
    where
        E: de::Error,
    {
        // Only our internal number/raw deserializers emit these byte keys.
        // Serde Content preserves bytes separately from strings, so tagged
        // enum buffering cannot turn a literal object into a synthetic value.
        match s {
            #[cfg(feature = "arbitrary_precision")]
            s if s == crate::number::TOKEN.as_bytes() => Ok(KeyClass::Number),
            #[cfg(feature = "raw_value")]
            s if s == crate::raw::TOKEN.as_bytes() => Ok(KeyClass::RawValue),
            _ => Err(de::Error::invalid_type(Unexpected::Bytes(s), &self)),
        }
    }
}
