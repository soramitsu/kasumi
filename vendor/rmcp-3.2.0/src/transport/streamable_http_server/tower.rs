...
use super::session::{
    ...
    SessionState, SessionStore, never::NeverSessionManager,
};
use crate::{
    RoleServer,
    model::{
        ...
        ProtocolVersion, RequestId, RequestMetaObject, ServerInfo, ServerJsonRpcMessage,
        ServerResult,
    },
    serve_server,
    service::{
        AtomicU32RequestIdProvider, NotificationContext, ORIGINATING_REQUEST, Peer, RequestContext,
        Service, negotiate_protocol_version, serve_directly_with_ct, uses_legacy_lifecycle,
    },
    transport::{
...
/// A terminal-only transport withheld the handler's response. HTTP adapters
/// must preserve uncertain mutation outcomes when this extension is present.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[non_exhaustive]
pub enum TerminalTransportFailure {
    Cancelled,
    UnsupportedPeerTraffic,
    InvalidConfiguration,
}

fn terminal_configuration(config: &StreamableHttpServerConfig) -> bool {
    !config.legacy_session_mode
        && config.json_response
        && config.stateless_protocol_metadata_required
        && config.session_store.is_none()
        && !config.allowed_hosts.is_empty()
        && config.max_request_body_bytes > 0
}

fn terminal_failure(id: Option<RequestId>, failure: TerminalTransportFailure) -> BoxResponse {
    let mut response = jsonrpc_message_response(
        ServerJsonRpcMessage::error(
            ErrorData::internal_error("terminal HTTP transport cannot release this response", None),
            id,
        ),
        true,
    )
...
impl<S> StreamableHttpService<S, NeverSessionManager>
where
    S: crate::ServerHandler + Send + 'static,
{
    /// Execute each terminal stateless request inside the calling HTTP future.
    /// No service loop, handler, response-send or waiter task is spawned. The
    /// caller owns cancellation, panic observation and draining of that future.
    /// Outbound peer messages and inbound non-request messages are rejected;
    /// this transport never switches to SSE or a session-based execution path.
    pub fn new_terminal_stateless(
        service_factory: impl Fn() -> Result<S, std::io::Error> + Send + Sync + 'static,
        config: StreamableHttpServerConfig,
    ) -> Result<Self, std::io::Error> {
        if !terminal_configuration(&config) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "terminal HTTP requires explicit hosts, bounded bodies, stateless metadata, JSON and no session store",
            ));
        }
    ...
    M: SessionManager,
{
    ...
    async fn serve_terminal_request_inline(
        &self,
        service: S,
        ...
        let (peer, mut outbound) =
            Peer::<RoleServer>::new(Arc::new(AtomicU32RequestIdProvider::default()), peer_info);
        peer.require_request_metadata();
        let cancellation = self.config.cancellation_token.child_token();
        // Dropping the actual HTTP future also cancels any cloned context proof;
        // the handler future itself is a child future, never an independent task.
        let _disconnect = cancellation.clone().drop_guard();
        let context = RequestContext {
            ct: cancellation.clone(),
            id: request.id.clone(),
            meta: std::mem::replace(request.request.get_meta_mut(), RequestMetaObject::new()),
            extensions: std::mem::take(request.request.extensions_mut()),
            peer,
        };
        let service = NegotiatingStatelessHttpService(service);
        let handler = ORIGINATING_REQUEST.scope(
            request.id.clone(),
            service.handle_request(request.request, context),
        );
        tokio::pin!(handler);
        let result = tokio::select! {
            biased;
            _ = cancellation.cancelled() => {
                return Err(terminal_failure(Some(request.id), TerminalTransportFailure::Cancelled));
            }
            message = outbound.recv() => {
                // The handler owns a Peer, so this channel cannot close before
                // it completes. Either outcome is unsupported transport use.
                drop(message);
                return Err(terminal_failure(Some(request.id), TerminalTransportFailure::UnsupportedPeerTraffic));
            }
        ...
        // A synchronous peer enqueue may occur in the same poll that completes
        // the handler. Do not discard it and return a misleading success.
        if outbound.try_recv().is_ok() {
            return Err(terminal_failure(
                Some(request.id),
        ...
        jsonrpc_message_response(message, true)
    }
    ...
    where
        ...
        B::Error: Display,
    {
        // `config` is public upstream. A later mutation must not reopen a
        // spawned/session path on a terminal-only service or its clones.
        if self.terminal_only && !terminal_configuration(&self.config) {
            return terminal_failure(None, TerminalTransportFailure::InvalidConfiguration);
        }
        if let Err(response) =
            ...
            match message {
                ClientJsonRpcMessage::Request(mut request) => {
                    if self.terminal_only {
                        return self
                            .serve_terminal_request_inline(service, request, part)
                            .await;
                    }
                ...
                ClientJsonRpcMessage::Notification(_notification) => {
                    if self.terminal_only {
                        return Err(terminal_non_request_response());
                    }
                    // ignore
                    Ok(accepted_response())
                }
                ClientJsonRpcMessage::Response(_json_rpc_response) => {
                    if self.terminal_only {
                        Err(terminal_non_request_response())
                    } else {
                        Ok(accepted_response())
                    }
                }
                ClientJsonRpcMessage::Error(_json_rpc_error) => {
                    if self.terminal_only {
                        Err(terminal_non_request_response())
                    } else {
                        Ok(accepted_response())
...
fn terminal_non_request_response() -> BoxResponse {
    let error = ErrorData::invalid_request(
        "terminal HTTP requires one JSON-RPC request; asynchronous peer messages are unsupported",
        None,
    );
    let mut response = jsonrpc_message_response(ServerJsonRpcMessage::error(error, None), true)
        .unwrap_or_else(|response| response);
...
#[cfg(test)]
#[path = "terminal_stateless_tests.rs"]
mod terminal_stateless_tests;
