...
pub enum BlockOperation {
    ...
    /// Hold initial vote loading to exercise cancellation before any task exists.
    ReadVote,
    BuildSnapshot,
    PurgeLog,
}
...
impl MemStore {
    ...
    pub fn set_panic_on_begin_receiving_snapshot(&self, panic: bool) {
        self.panic_on_begin_receiving_snapshot
...
impl RaftStorage<TypeConfig> for Arc<MemStore> {
    ...
    async fn read_vote(&mut self) -> Result<Option<Vote<MemNodeId>>, StorageError<MemNodeId>> {
        if let Some(duration) = self.get_blocking(&BlockOperation::ReadVote) {
    ...
    async fn begin_receiving_snapshot(
        ...
        if self
            .panic_on_begin_receiving_snapshot
    ...
    async fn install_snapshot(
        ...
        {
            let new_sm: MemStoreStateMachine =
