...
//!
//! Kasumi's security patch removes mutable byte access, which would allow an
//! invalid bool representation for a one-bit bitmap (RUSTSEC-2025-0167):
//!
//! ```compile_fail
//! use bitmaps::Bitmap;
//! let mut bitmap = Bitmap::<1>::new();
//! let bytes: &mut [u8] = bitmap.as_mut();
//! bytes[0] = 255;
//! ```
