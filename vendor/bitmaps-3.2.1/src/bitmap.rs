...
// Kasumi security patch for RUSTSEC-2025-0167: mutable byte access is deliberately
// not implemented. Bitmap<1> stores a bool, which does not permit arbitrary
// byte values. Callers must use checked construction or the bit accessors.
...
mod test {
    ...
    #[test]
    fn one_bit_decode_rejects_all_invalid_bool_representations() {
        for byte in 0..=u8::MAX {
            let decoded = Bitmap::<1>::try_from([byte].as_slice());
            if byte <= 1 {
                assert_eq!(decoded.unwrap().get(0), byte == 1);
            } else {
                assert!(decoded.is_err(), "accepted invalid bool byte {byte}");
            }
        }
        assert!(Bitmap::<1>::try_from([].as_slice()).is_err());
        assert!(Bitmap::<1>::try_from([0, 0].as_slice()).is_err());
        assert_eq!(Bitmap::<8>::try_from([255].as_slice()).unwrap().len(), 8);
    }
