...
impl NodeDiskDirectory {
    ...
    /// Return the enrolled directory's device and inode after checking its
    /// retained descriptor, complete ancestor path and installed ledger.
    /// The observation carries no descriptor custody or write authority.
    pub fn verified_identity(&self) -> io::Result<(u64, u64)> {
        ...
        let Identity(device, inode) = self.owner().identity;
        Ok((device, inode))
    }
