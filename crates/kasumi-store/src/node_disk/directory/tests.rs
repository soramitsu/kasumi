...
#[test]
fn verified_identity_uses_enrolled_directory_and_fences_path_substitution() {
    let (directory, config, memory) = fixture();
    let path = directory.path().join("backup");
    crate::private_files::create_directory(&path).unwrap();
    let disk = open(&config, &memory);
    let handle = disk.open_directory("fixture", Path::new("backup")).unwrap();
    let metadata = path.metadata().unwrap();
    assert_eq!(
        handle.verified_identity().unwrap(),
        (metadata.dev(), metadata.ino())
    );
    assert_eq!(disk.snapshot().phase, NodeDiskPhase::Open);

    std::fs::rename(&path, directory.path().join("retained")).unwrap();
    crate::private_files::create_directory(&path).unwrap();
    assert!(handle.verified_identity().is_err());
