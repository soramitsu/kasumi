...
fn managed_child_names_depth_existing_identity_and_root_removal_are_exact() {
    let (directory, mut config, memory) = fixture();
    // Ancestor locks follow the physical path. On macOS a fixture path under
    // /var resolves through /private/var and needs one more ancestor.
    config.max_depth = u32::try_from(
...
fn generic_nonroot_open_uses_retained_parent_walk_and_exact_identity() {
    ...
    assert_eq!(
        disk.open_directory("fixture", Path::new("a/missing"))
            .unwrap_err()
            .kind(),
        io::ErrorKind::NotFound
    );
...
fn generic_nonroot_open_retains_walk_close_failure_before_publication() {
    ...
    assert_eq!(
        operation.close_failure.unwrap().step,
        NodeDiskDirectoryOperationStep::Verify
    );
    assert_eq!(disk.snapshot().open_directories, 1);
    assert!(disk.reconcile(&CensusCancellation::default()).is_err());
