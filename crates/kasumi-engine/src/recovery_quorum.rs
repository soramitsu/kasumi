...
pub(crate) fn completion_route_retry(
    ...
    matches!((&pending.input,input),
        ...
        if matches!(pending.phase,RecoveryPhase::Complete|RecoveryPhase::Confirm) && old!=new && original==next && now<original.not_after_ms && matches!(original.step,TargetRuntimeStep::Complete(_)|TargetRuntimeStep::PrepareComplete(_)|TargetRuntimeStep::InspectCompletionAttempt(_)|TargetRuntimeStep::InspectCompletionResolution(_)|TargetRuntimeStep::ResolveComplete(_)|TargetRuntimeStep::Inspect(_)|TargetRuntimeStep::Activate{..}))
}
