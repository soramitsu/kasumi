    ...
    > {
        match self {
            ...
            Self::Captured(generation) if kind == 21 => {
                ...
                Ok(Box::new(generation.terminals.records().map(|row| row.map(|row| crate::snapshot_codec::Record::Terminal(Box::new(row))))
                    .filter(move |record| primary.as_ref().is_none_or(|key| record.as_ref().map_or(true, |record| record.order().1 == *key)))))
            }
