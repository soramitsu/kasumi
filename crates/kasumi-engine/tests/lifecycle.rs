...
struct Fixture {
    ...
    physical: BTreeMap<u64, common::PhysicalFixture>,
    router: Arc<InProcessRouter>,
    vote_probe: Arc<BootstrapVoteProbe>,
...
impl Fixture {
    ...
    async fn configured(limits: Limits, max_state_bytes: usize) -> Self {
        ...
        let vote_probe = Arc::new(BootstrapVoteProbe::new(router.clone()));
        let mut result = Self {
            root,
            physical,
            router,
    ...
    async fn open(&mut self, create: bool) {
        for id in 1..=3 {
            ...
            let audit = if create {
                ...
                common::existing_security_audit(
                    node.clone(),
                    self.physical[&id].storage.admission.clone(),
                )
                .await
            };
            let store = (if create {
    ...
    async fn leader(&self) -> Arc<Database> {
        ...
        .unwrap_or_else(|error| {
            panic!(
                "control leader wait failed: {error}; states: {:?}; vote_transport={}",
                self.nodes
                    .iter()
                    .map(|(id, db)| (
                        *id,
                        format!("{:?}", db.raft_group().raft().metrics().borrow())
                    ))
                    .collect::<Vec<_>>(),
                self.vote_probe.diagnostic()
            )
        })
    }
...
async fn control_completion_audit_reservation_survives_denials_and_current_admin_status_recovery() {
    ...
    assert_eq!(
        after.state.audit_retention.next_sequence,
        before_sequence + 1
    );
    ...
    assert_eq!(
        event.event_id,
        format!("{}:{}", after.state.incarnation, begin.revision)
    );
