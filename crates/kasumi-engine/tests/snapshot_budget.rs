...
fn engine(limit: usize) -> TenantEngine {
    TenantEngine::new(
        "tenant".into(),
        "incarnation".into(),
        Policy {
...
fn replay_with_only_rejection_audit_headroom_keeps_the_original_receipt() {
    ...
    loop {
        let limit = serde_json::to_vec(&after).unwrap().len() + 19 - 1;
    ...
    let db = engine(16 << 10);
    db.restore(&serde_json::to_vec(&before).unwrap()).unwrap();
    assert_eq!(
...
fn recovery_and_limit_changes_cannot_admit_state_above_snapshot_format_or_tenant_budget() {
    ...
    assert!(db.restore(&serde_json::to_vec(&state).unwrap()).is_err());
    ...
    invalid.max_snapshot_bytes = MAX_TENANT_SNAPSHOT_BYTES + 1;
    assert_eq!(
