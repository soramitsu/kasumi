...
impl KasumiRecoveryClient {
    ...
    pub async fn start(
        ...
        let response = self.inner.start(wire(bearer, request)?).await?.into_inner();
    ...
    pub async fn stop(
        ...
        let response = self.inner.stop(wire(bearer, request)?).await?.into_inner();
        let record = head(response, request.operation_id)?;
    ...
    pub async fn read_phase(
        ...
        let response = self
            .inner
            .read_phase(wire(bearer, request)?)
            .await?
            .into_inner();
