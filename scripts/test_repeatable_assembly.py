...
class AssemblyCustodyTests(unittest.TestCase):
    ...
    def test_frozen_roster_rejects_omitted_transitive_inputs_and_unrelated_blobs(self):
        functional = {"python_executable": {"artifact": "tools/python"}, "gates": [
            {"name": "format", "log": "format.log", "process": "format-process.json",
             "resources": "format-resources.json"},
            {"name": "production", "log": "production.log", "process": "production-process.json",
             "resources": "production-resources.json", "executables": {"release/kasumid": {}}},
        ]}
        source_files = {"Cargo.lock": {}, ".cargo/config.toml": {}}
        production = functional["gates"][1]
        expected = {"evidence.json", "source-files.json", "source.tar", "tools/python",
                    "format.log", "format-process.json", "format-resources.json",
                    "production.log", "production-process.json", "production-resources.json",
                    "target/release/kasumid", "source/Cargo.lock", "source/.cargo/config.toml"}
        frozen = {name: {} for name in expected}
        assembly.check_frozen_input_roster(frozen, functional, production, source_files)
        for missing in ("format.log", "format-process.json", "format-resources.json",
                        "target/release/kasumid", "source/.cargo/config.toml", "tools/python", "source.tar"):
            with self.subTest(missing=missing), self.assertRaisesRegex(ValueError, "frozen input roster differs"):
                assembly.check_frozen_input_roster({name: {} for name in expected - {missing}},
                                                   functional, production, source_files)
        with self.assertRaisesRegex(ValueError, "frozen input roster differs"):
            assembly.check_frozen_input_roster({**frozen, "target/debug/unrelated": {}},
                                               functional, production, source_files)
    ...
    def test_documented_frozen_dispatch_binds_imports_and_rejects_checkout_dispatch(self):
        ...
        self.assertNotIn("source/scripts/repeatable_assembly.py", workflow)
        self.assertIn("--output /evidence/repeatable-assembly", workflow)
        self.assertIn('--output "$KASUMI_RUN/repeatable-assembly"', workflow)
        self.assertIn("${{ runner.temp }}/${{ env.KASUMI_RUN_NAME }}/repeatable-assembly/**", workflow)
        source_scripts = Path(__file__).resolve().parent
        # Actual subprocesses import identical module bytes from two locations.
        # Only the declared frozen source is admissible. No native tool or Cargo
        # process is run, and these are not full candidate assembly fixtures.
        checkout = self.root / "checkout"
        frozen = self.root / "frozen-source"
        for directory in (checkout, frozen):
            ...
            for name in ("run_repeatable_assembly_owned.py", "repeatable_assembly.py", "assembly_inputs.py",
                ...
                if (source_scripts / name).exists():
                    ...
                    original = Path(assembly.gate_process.__file__).parent / name
                shutil.copyfile(original, scripts / name)
            (scripts / "probe.py").write_text(
                "from pathlib import Path\nimport sys\nimport run_repeatable_assembly_owned, repeatable_assembly, assembly_inputs\n"
                ...
                "  if path.is_relative_to(runtime):files[str(path)]=assembly_inputs.file_identity(path)\n"
                "assembly_inputs.consumed({'roots': {}}, source, {'packages': []}, files)\n"
                "print('frozen dispatch bound before any native child')\n")
        for directory, passes in ((frozen, True), (checkout, False)):
            command = [self.inputs["tools"]["python"]["path"], "-B", "-S", str(directory / "scripts/probe.py"), str(frozen)]
            item = assembly.run_owned(self.custody, directory.name, command, str(checkout), self.environment, 10)
            process = assembly.read(assembly.check_ref(self.custody, item["receipt"]))
            self.assertEqual(process["exit_code"] == 0, passes)
            self.assertTrue(process["cleanup"]["drained"])
            if passes:
                self.assertIn(b"bound before any native child", assembly.check_ref(self.custody, item["stdout"]).read_bytes())
            else:
                self.assertIn(b"Python imported unbound module", assembly.check_ref(self.custody, item["stderr"]).read_bytes())
