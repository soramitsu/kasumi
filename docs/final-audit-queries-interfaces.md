...
| Shared embedded/native/MCP authorization and exact JSON serialization | Native methods and MCP tools call the same [Database service](../crates/kasumi-engine/src/service.rs). [Protobuf](../crates/kasumi-client/proto/kasumi.proto) uses UTF-8 JSON bytes. The adapter round-trip test verifies `90071992547409931234567890.123456789`, receipt replay, queries and discovery through both adapters; cross-tenant/scope/admin tests require matching denials. | Covered. The embedding application is trusted to supply its authenticated context. |
...
| Data/schema discovery, point/query/atomic mutations/receipt lookup; separate administration | The audited baseline had five native data methods matching five MCP tools; administration uses a distinct service/listener and [CLI](../crates/kasumi-server/src/bin/kasumictl.rs). Adapter tests reject admin/control route escalation. The current [native contract](../crates/kasumi-client/proto/kasumi.proto) additionally provides `ReadSnapshot`, covered separately in [transaction additions](transactions.md). | Covered for the recorded baseline. Storage/security audit covers the administrative lifecycle itself. |
