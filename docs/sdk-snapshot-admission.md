...
Admission precedes request traversal, encoding, expected-point metadata cloning, pool routing and dispatch. Outbound serialization first walks borrowed values without copying them, bounding depth, nodes and string/request work; the output writer rejects its byte limit before growing its buffer. Retries keep the original deadline, credential snapshot and member pin. Each retry acquires another reservation if a cancelled attempt still owns transport or decoder work.
