...
```rust,ignore
Database::backup_checkpoint(context, &dyn BackupDestination, session_id)
Database::verify_backup_checkpoint(context, &dyn BackupDestination, backup_id)
Database::backup_checkpoint_named(context, "approved-destination", session_id)
...
`key_lineage_digest` commits to the authenticated catalog stream in canonical graph traversal order using a domain-separated SHA-256 digest and checked 64-bit final count. It includes the permanent session catalog, root, pages, resident chunks, and cold-history dependencies without accumulating a deduplication set. See [durable backup sessions](backup-sessions.md) for the exact traversal and session ownership contract. Each catalog digest binds active key identity and wrapped dependencies; no plaintext keys are included.
