...
A creation timeout can leave immutable published objects and returns UnknownOutcome. A readback timeout does not change backup contents. Automatic orphan cleanup and a client-selected permanent backup-creation identity are not implemented. Existing backup format bounds still apply: at most 2 GiB resident state, 8 MiB chunks, 4 MiB manifest and the admitted resident verification workspace. This implementation does not establish capacity or recovery-time objectives for 10,000 tenants, independent fencing when the source quorum is unavailable, or Linux release qualification.
