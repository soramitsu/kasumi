...
out=pathlib.Path('/tmp/kasumi-combined-075e24d-source-gates')
...
env=os.environ.copy();env.update(RUSTUP_TOOLCHAIN='1.97.1',RUSTC=rustc,RUSTDOC=rustdoc,CARGO_TARGET_DIR='/tmp/kasumi-production-root-target',CARGO_BUILD_JOBS='1',RUST_TEST_THREADS='1');env['PYTHONDONTWRITEBYTECODE']='1';env['PATH']=str(pathlib.Path(cargo).parent)+':'+env['PATH']
record={'scope':'Combined frozen source release-tool Python tests, exact dependency resolver/input hash checks and workspace formatting. No Rust compilation, native listener, provider, final release or capacity acceptance.','source_before':before,'tools':{str(p):sha(p) for p in [cargo,rustc,rustdoc]},'runner_sha256':sha(__file__),'per_gate_timeout_seconds':300,'gates':[]}
(out/'evidence.json').write_text(json.dumps(record,indent=2))
python='/Users/mtakemiya/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3'
commands=[('python-tools',[python,'-B','-m','unittest','discover','-s','scripts','-p','test_*.py','-v'],None),('dependency-patches',[python,'-B','scripts/check_dependency_patches.py'],None),('format',[cargo,'fmt','--all','--','--check'],None)]
