...
root=Path('/Users/mtakemiya/dev/kasumi');pkg=Path(__file__).resolve().parent.parent;expected_manifest='83ab0cf18c5ee3d5b4b00309c4073200faa1d753254b9f871da2837018b4faec';out=pkg/'native-01';assembly=pkg/'assembly';target=out/'cargo-target'
...
def inventory():
 ...
 paths += [pkg/n for n in ['manifest.json','combined.patch','composition.json','expected-store-tests.json','revision6-to-combined.patch','custody-to-combined-overlaps.patch','correction.patch','changed-paths.json','apply-readback.json','integration-gaps.json']]+[Path(__file__)]
...
commands[0],commands[1]=commands[1],commands[0] # Strict lint first; every original gate remains required.
