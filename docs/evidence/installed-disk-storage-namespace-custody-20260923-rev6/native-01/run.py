...
root=Path('/Users/mtakemiya/dev/kasumi');pkg=Path(__file__).resolve().parent.parent;expected_manifest='b2d6f9f980a55d0e1a688093cf2deb64269878005ff78e15f394af8341b336e5';out=pkg/'native-01';assembly=pkg/'assembly';target=out/'cargo-target'
...
def qualified_stage(result):
 return (result['exit_code']==0 and result['drained'] and not result['timeout'] and not result['signals']
         ...
         and result.get('inventory_passed',True) and result.get('expected_python_3_12',True) and result.get('focus_passed',True))
...
def inventory():
 ...
 paths += [pkg/n for n in ['manifest.json','combined.patch','composition.json','expected-store-tests.json','revision6-to-combined.patch','custody-to-combined-overlaps.patch','correction.patch','changed-paths.json','apply-readback.json','integration-gaps.json','focused-test-plan.json']]+[Path(__file__)]
...
expected_record=json.loads((pkg/'expected-store-tests.json').read_text());expected=set(expected_record['expected']);assert len(expected)==338
focus_plan=json.loads((pkg/'focused-test-plan.json').read_text());assert len(focus_plan['focused_names'])==14;assert set(focus_plan['focused_names'])<=expected
commands=[('compile-store',[str(toolchain/'cargo'),'test','--offline','--locked','-p','kasumi-store','--all-features','--lib','--no-run','--message-format=json']),('store-inventory',['STORE_BINARY','--list'])]
commands += [(f'focused-{index:02d}', ['STORE_BINARY','--exact',case,'--test-threads=1','--nocapture']) for index,case in enumerate(focus_plan['focused_names'],1)]
commands += [('workspace-clippy',[str(toolchain/'cargo'),'clippy','--offline','--locked','--workspace','--all-targets','--all-features','--message-format=json','--','-D','warnings']),('workspace-check',[str(toolchain/'cargo'),'check','--offline','--locked','--workspace','--all-targets','--all-features','--message-format=json']),('store-full',['STORE_BINARY','--test-threads=1','--nocapture'])]
...
for name,command in commands:
 ...
 if name=='store-inventory':
  actual=set(line.removesuffix(': test') for line in logpath.read_text().splitlines() if line.endswith(': test'));missing=sorted(expected-actual);extra=sorted(actual-expected);inventory_passed=not missing and not extra and len(actual)==338
 ...
 if name.startswith('focused-'):
  ...
  result['focused_test_name']=command[2]
  result['focus_passed']=bool(summaries) and tuple(map(int,summaries[-1]))==(1,0,0,0,len(expected)-1)
