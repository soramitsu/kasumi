...
OUTPUT = Path('/tmp/kasumi-stock-sdk-selection-diagnostic-3f88b44')
...
assert not TARGET.exists(), 'requires the previously confirmed independent cold target'
before = source_state()
...
gates = [
    ...
    ('canonical-selection-no-run', test_base + ['--no-run', '-p', 'kasumi-stock-json-sdk-consumer', '-p', 'kasumi-client', '--lib'] + ordered + [private, '--', '--exact', '--test-threads=1'], 'ordered', None),
]
report = {
    'schema': 1, 'scope': 'Authorized one external SDK-only compile selection diagnostic; no test execution, engine/store/server/Raft compilation or native listeners',
    ...
    'target': str(TARGET), 'initial_target_absent': True, 'jobs': 1, 'test_threads': 1, 'gates': [], 'status': 'running',
    ...
    'official_cargo_evidence': ['https://doc.rust-lang.org/cargo/reference/features.html#resolver-version-2-command-line-flags', 'https://doc.rust-lang.org/cargo/commands/cargo-test.html#package-selection'],
    'selection_feasibility': 'Selecting root consumer may authorize its namespaced feature while selecting the client dependency; official docs do not guarantee this mixed selection, so actual diagnostic is required',
