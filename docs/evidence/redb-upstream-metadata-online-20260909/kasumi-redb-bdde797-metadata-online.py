...
out=Path('/tmp/kasumi-redb-bdde797-metadata-online')
...
env=dict(os.environ,RUSTUP_TOOLCHAIN='1.97.1',RUSTC=str(cargo.parent/'rustc'),RUSTDOC=str(cargo.parent/'rustdoc'),CARGO_BUILD_JOBS='1',CARGO_NET_OFFLINE='false',CARGO_TARGET_DIR='/tmp/kasumi-redb-metadata-no-build-target',PYTHONDONTWRITEBYTECODE='1')
record={'scope':'Network-enabled metadata-only dependency preparation after preserved offline cache failure for exact complete upstream source overlay; no compiler/native/test claim. Root and fuzz graphs are independently attempted, each300s, source and helper hashes guarded, changes restricted to verification locks. No workspace members or verification dependencies are removed.','source':git('rev-parse','HEAD'),'tree':git('rev-parse','HEAD^{tree}'),'files_before':before,'runner_sha256':sha(Path(__file__)),'helper_sha256':sha(helper),'tools':{str(p):sha(p) for p in [cargo,cargo.parent/'rustc',cargo.parent/'rustdoc']},'gates':[]}
def persist():
...
for name,manifest,lock in [('upstream','Cargo.toml','Cargo.lock'),('fuzz','fuzz/Cargo.toml','fuzz/Cargo.lock')]:
    ...
    command=[str(cargo),'metadata','--all-features','--format-version','1','--manifest-path',manifest]
    ...
    with (out/(name+'.log')).open('wb') as stream:
        value=mod.run(command,root,env,stream,300,observe)
