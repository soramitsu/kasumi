...
expected_binary='4ad75c3d3586d0464e9326ff32b9f38aae1c84a991b16a52087ef80f02d498de'
def binary_digest():
...
before=inventory(); (out/'139-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
...
def run():
 ...
 with (out/'139-lifecycle-handoff.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'139-lifecycle-handoff.log')}),flush=True)
 ...
 after=inventory(); result={'scope':'Both original129 uncertain-activation cases after explicit stale-route Arc retirement; original workload/assertions/deadlines, no final acceptance','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'inventoried_source_unchanged':before==after,'binary_path':str(binary),'expected_binary_sha256':expected_binary,'binary_sha256_before':binary_before,'binary_sha256_after':binary_after,'binary_unchanged':binary_before==binary_after}
 (out/'139-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'139-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
 lines=(out/'139-lifecycle-handoff.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
...
try:
 ...
 (out/'139-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'139-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
