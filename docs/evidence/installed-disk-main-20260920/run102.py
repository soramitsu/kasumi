...
before=inventory(); (out/'102-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
command=['/Users/mtakemiya/.rustup/toolchains/1.97.1-aarch64-apple-darwin/bin/cargo', 'test', '--manifest-path', 'vendor/redb-4.2.0/Cargo.toml', '--offline', '--locked', '--lib', 'retained_transaction', '--', '--test-threads=1', '--nocapture']
...
def run():
 ...
 with (out/'102-retained-redb-terminal.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'102-retained-redb-terminal.log')}),flush=True)
 ...
 after=inventory(); result={'scope':'development actual-backend retained redb terminal prerequisite tests; no production caller adoption or release qualification','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'inventoried_source_unchanged':before==after}
 (out/'102-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'102-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
 lines=(out/'102-retained-redb-terminal.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
 raise SystemExit(rc)
...
try:
 ...
 (out/'102-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'102-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
