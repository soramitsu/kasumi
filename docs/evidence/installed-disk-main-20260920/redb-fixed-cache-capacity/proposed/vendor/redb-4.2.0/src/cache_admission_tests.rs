...
use crate::{
    AdmissionError, CloseError, CommitError, Database, OwnerFailed, ReadableDatabase,
    RetainedWriteTransaction, StorageAdmission, StorageError, TableDefinition, TerminalObservation,
    WriteTerminalError, WriteTerminalSettlement,
};
use std::sync::{
    Arc, Mutex,
...
fn cache_denial_keeps_original_error_and_failed_or_panicked_abort() {
    for mode in [1u8, 2] {
        ...
        let database = match f.db.close() {
            Err(CloseError::Busy(db)) => db,
            _ => panic!("actual retained tx must keep DB busy"),
        };
