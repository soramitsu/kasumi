...
before=inventory(); (out/'62-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
command=['/Users/mtakemiya/dev/kasumi/target/debug/deps/kasumi_engine-dc6dca13bbdbe0d0', '--test-threads=1', '--nocapture', 'admission::startup_integration_tests']
assert hashlib.sha256(Path(command[0]).read_bytes()).hexdigest()=='bd5ff3623872258a02c606ddc3f1bfbae627004575e94d2b24f148a0710908ee'
...
def run():
 ...
 with (out/'62-actual-startup.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'62-actual-startup.log')}),flush=True)
 ...
 after=inventory(); result={'scope':'development actual cancelled Raft startup and census regression using gate61 test binary; concurrent with gate53, not a performance measurement, not final release qualification','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'binary_sha256':'bd5ff3623872258a02c606ddc3f1bfbae627004575e94d2b24f148a0710908ee','binary_unchanged':hashlib.sha256(Path(command[0]).read_bytes()).hexdigest()=='bd5ff3623872258a02c606ddc3f1bfbae627004575e94d2b24f148a0710908ee','process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'inventoried_source_unchanged':before==after}
 (out/'62-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'62-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
 lines=(out/'62-actual-startup.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
 raise SystemExit(rc)
...
try:
 ...
 (out/'62-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'62-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
