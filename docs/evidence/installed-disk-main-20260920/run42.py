...
before=inventory(); (out/'42-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
...
def run():
 ...
 with (out/'42-workspace-clippy.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'42-workspace-clippy.log')}),flush=True)
 ...
 (out/'42-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'42-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
 lines=(out/'42-workspace-clippy.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
 raise SystemExit(rc)
...
try:
 ...
 (out/'42-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'42-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
