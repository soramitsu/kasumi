...
r=Path.cwd();p=r/'target/installed-disk-validation/directory-cursor';prior=r/'target/installed-disk-validation/directory-parent-transitions'
assert subprocess.check_output(['git','branch','--show-current'],text=True).strip()=='master'
def sha(f):return hashlib.sha256(f.read_bytes()).hexdigest()
assert sha(prior/'cumulative.patch')=='5ae0623c48dbaf32af81544e7c3fafb2d6f25c84aab6f385c7e3a0b7e85ae3bd'
shutil.copytree(prior/'proposed',p/'cumulative-proposed');shutil.copytree(prior/'cumulative-base',p/'cumulative-base')
...
rel=Path('crates/kasumi-store/src/backup_sessions_fs.rs');q=p/'cumulative-base'/rel;q.parent.mkdir(parents=True,exist_ok=True);shutil.copyfile(p/'base'/rel,q)
...
patches={'cursor.patch':patch(p/'base',p/'proposed','cursor.patch'),'cumulative.patch':patch(p/'cumulative-base',p/'cumulative-proposed','cumulative.patch')}
...
for f in sorted((p/'cumulative-proposed').rglob('*')):
 ...
 manifest['files'][str(rel)]={'successor_changed':(p/'proposed'/rel).exists(),'successor_base_sha256':sha(baseline) if baseline.exists() else None,'cumulative_base_sha256':sha(cb) if cb.exists() else None,'current_actual_sha256':sha(actual) if actual.exists() else None,'proposed_sha256':sha(f)}
