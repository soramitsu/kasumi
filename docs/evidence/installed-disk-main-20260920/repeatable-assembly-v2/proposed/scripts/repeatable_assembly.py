...
def evidence_inputs(evidence, functional, production):
    ...
    selected.update("source/" + relative for relative in inventory(evidence / "source"))
...
def domain_adapter(root, domain, files, configs, artifacts):
    """Acceptance bridge, deliberately unregistered pending native runner validation."""
    import verify_release_acceptance as acceptance
    exact(domain["details"], {"report", "second_package", "second_source"}, "assembly domain details")
    report_path = acceptance.reference(root, domain["details"]["report"])
    attempt_root = report_path.parent
    parsed = verify(attempt_root, ref(attempt_root, report_path))
    ...
    require(domain["runner"] == {"source_path": RUNNER, "sha256": files[RUNNER]["sha256"]},
            "assembly domain runner differs from fixed contract")
    require(domain["started_at"] == record["started_at"] and domain["finished_at"] == record["finished_at"],
