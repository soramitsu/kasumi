...
import shutil,json
r=Path.cwd();p=r/'target/installed-disk-validation/directory-parent-transitions';d=p/'native-01';d.mkdir();s=p/'proposed/crates/kasumi-store/src';a=r/'crates/kasumi-store/src'
(d/'deps').mkdir();names='anyhow libc serde serde_core serde_derive sha2 cfg_if cpufeatures digest block_buffer crypto_common generic_array typenum uuid getrandom rand rand_core parking_lot parking_lot_core lock_api scopeguard smallvec zerocopy'.split();selected={};copied={}
for n in names:
 choices=[f for f in (r/'target/debug/deps').glob('lib'+n+'-*') if f.suffix in ('.rlib','.dylib')]
 for f in choices:
  shutil.copyfile(f,d/'deps'/f.name);copied[str(f)]=str(d/'deps'/f.name)
 libs=[f for f in choices if f.suffix=='.rlib']
 if libs:selected[n]=str(d/'deps'/max(libs,key=lambda f:f.stat().st_mtime_ns).name)
(d/'selected.json').write_text(json.dumps(selected,indent=2)+'\n');(d/'copied-dependencies.json').write_text(json.dumps(copied,indent=2)+'\n')
# Freeze unchanged implementation imports as exact source, too.
for rel in ('disk_memory.rs','device_disk.rs'):
 (d/rel).write_bytes((a/rel).read_bytes())
prefix='#![allow(dead_code)]\n'
for mod,path in [('disk_memory',d/'disk_memory.rs'),('device_disk',d/'device_disk.rs'),('node_disk',s/'node_disk.rs')]:prefix+=f'#[path = {json.dumps(str(path))}] mod {mod};\n'
prefix+='pub use disk_memory::DiskOpenError;\n'
body='''use std::{collections::BTreeMap, path::{Path,PathBuf}, sync::Arc, os::unix::fs::{DirBuilderExt, MetadataExt}};
...
'''
(d/'driver.rs').write_text(prefix+body)
