...
inputs=[f for folder in (pkg/'proposed',p/'deps') for f in folder.rglob('*') if f.is_file()]+[p/x for x in ('driver.rs','disk_memory.rs','device_disk.rs','selected.json','copied-dependencies.json','run.py')]+[compiler]
