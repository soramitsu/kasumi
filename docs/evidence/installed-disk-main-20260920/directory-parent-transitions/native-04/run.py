...
inputs=[f for folder in (pkg/'proposed',p/'deps',p/'node_disk') for f in folder.rglob('*') if f.is_file()]+[p/x for x in ('private_files.rs','node_disk.rs','driver.rs','disk_memory.rs','device_disk.rs','selected.json','copied-dependencies.json','run.py')]+[compiler]
...
for name in ('anyhow','serde','uuid','libc','sha2','parking_lot','zeroize','tempfile'):extern+=['--extern',name+'='+selected[name]]
