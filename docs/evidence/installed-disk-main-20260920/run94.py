...
before=inventory(); (out/'94-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
command=['/Users/mtakemiya/.rustup/toolchains/1.97.1-aarch64-apple-darwin/bin/cargo', 'test', '--offline', '--locked', '--no-fail-fast', '-p', 'kasumi-engine', '--all-features', '--lib', '--', '--test-threads=1', '--nocapture', '--skip', 'public_restore_admits_resident_state_without_charging_permanent_stream_as_ram']
...
def run():
 ...
 with (out/'94-engine-callers.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'94-engine-callers.log')}),flush=True)
 ...
 after=inventory(); result={'scope':'development engine library successor after five precise fixture corrections and complete file retirement; known unresolved 512x256 restore case explicitly excluded, not final release qualification','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'inventoried_source_unchanged':before==after}
 (out/'94-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'94-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
 lines=(out/'94-engine-callers.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
 raise SystemExit(rc)
...
try:
 ...
 (out/'94-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'94-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
