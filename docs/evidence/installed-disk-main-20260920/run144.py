...
before=inventory(); (out/'144-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
...
def run():
 ...
 with (out/'144-vendor-canonical-reclaim.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'144-vendor-canonical-reclaim.log')}),flush=True)
 ...
 after=inventory(); result={'scope':'Complete vendor redb library after bounded allocation400 purge with DATA400 and obsolete helper removal; no final acceptance','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'inventoried_source_unchanged':before==after}
 (out/'144-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'144-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
 lines=(out/'144-vendor-canonical-reclaim.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
 raise SystemExit(rc)
...
try:
 ...
 (out/'144-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'144-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
