...
fn validate_leaf(bytes: &[u8], count: usize, previous: &mut Option<AllocatorStateKey>) -> Result {
...
fn validate_page(
    ...
    match prefix[0] {
        LEAF => validate_leaf(bytes, count, previous),
        BRANCH => {
            ...
            for raw in keys.chunks_exact(5) {
                let key = AllocatorStateKey::checked(raw)?;
                if last_separator.is_some_and(|last| last >= key) {
                    return Err(malformed());
                }
            ...
            for raw in raw_children.chunks_exact(PageNumber::serialized_size()) {
                ...
                validate_page(child, resolver, path, depth + 1, previous)?;
            }
            Ok(())
        }
        _ => Err(malformed()),
...
pub(crate) fn validate_allocator_state_keys(
    ...
    if let Some(root) = root {
        validate_page(root.root, resolver, &mut path, 0, &mut previous)?;
    }
    Ok(())
}
...
mod tests {
    ...
    fn allocator_raw_leaf_geometry_is_checked_before_key_or_value_access() {
        ...
        validate_leaf(&valid, 1, &mut None).unwrap();
        for end in 0..valid.len() {
            assert!(matches!(
                validate_leaf(&valid[..end], 1, &mut None),
                Err(StorageError::Corrupted(_))
            ));
        }
        let mut backwards = valid.clone();
        backwards[4..8].copy_from_slice(&12_u32.to_le_bytes());
        assert!(validate_leaf(&backwards, 1, &mut None).is_err());
        let mut too_far = valid.clone();
        too_far[4..8].copy_from_slice(&u32::MAX.to_le_bytes());
        assert!(validate_leaf(&too_far, 1, &mut None).is_err());
        for size in [0, 7, 9] {
            assert!(validate_leaf(&leaf([5, 0, 0, 0, 0], &vec![0; size]), 1, &mut None).is_err());
        }
        let mut previous = Some(AllocatorStateKey::TransactionId);
        assert!(validate_leaf(&valid, 1, &mut previous).is_err());
    }
    ...
    fn allocator_raw_walk_rejects_bad_branch_separator_and_cycle_without_comparison() {
        ...
        validate_allocator_state_keys(root, &allocator.resolver()).unwrap();

        let mut branch = allocator.get_page_mut(number).unwrap();
        branch.memory_mut()[56] = 255;
        drop(branch);
        assert!(matches!(
            validate_allocator_state_keys(root, &allocator.resolver()),
            Err(StorageError::Corrupted(_))
        ));
        let mut branch = allocator.get_page_mut(number).unwrap();
