...
impl NodeDisk {
    ...
    fn open_inner(
        ...
        if let Some(existing) = installed.find(|entry| entry.config().is_some_and(|installed| installed.roots == config.roots)) {
        ...
        for entry in installed.iter() {
            disk_memory::require(
                entry.roots().is_none_or(|old_roots| !old_roots.values()
        ...
        installed.insert(List::prepare(RegisteredDisk {
            ...
            registration: Registration::Preparing(PreparedResources {
                config: config.clone(), memory, roots, ancestor_locks, accounted,
                live, streams: census::Streams::default(), charge,
            }),
        ...
        let totals = census(&prepared.roots, &prepared.config, unit, cancel,
            prepared.accounted.stage(census_limit)?, &prepared.streams)?;
        cancel.check()?;
        prepared.accounted.commit_stage();
        let device = selection.open(identity.0, config.min_free_bytes, prepared.memory.clone())?;
        let PreparedResources { config, memory, roots, ancestor_locks, accounted, live, streams: _, charge } = registration.take();
        ...
        drop(promises);
        registration.commit(disk.clone());
        Ok(disk)
    }
    ...
    pub fn fixture_for_path(
        ...
        let existing = {
            ...
            installed
                .find(|entry| entry.config().is_some_and(|config| config.binding(path.as_ref()).is_ok()))
