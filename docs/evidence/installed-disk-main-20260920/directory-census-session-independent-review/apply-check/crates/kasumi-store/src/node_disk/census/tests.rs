use super::*;
use super::super::{DiskWork, NodeDisk, NodeDiskPhase, PreparingRegistration, PreparedResources, RegisteredDisk, Registration, fixed_map};
...
fn cancellation_retains_independent_native_close_error_without_retry() {
    ...
    assert_eq!(error.downcast_ref::<CensusCloseFailure>().unwrap().errno, libc::EIO);
...
fn registered_preparation(config: &NodeDiskConfig, memory: &Arc<TestDiskMemory>, installed: &mut super::super::List<RegisteredDisk>) -> (Identity, u64) {
    ...
    installed.insert(super::super::List::prepare(RegisteredDisk {
        identity,
        registration: Registration::Preparing(PreparedResources {
            config: config.clone(), memory: memory.clone(), roots, ancestor_locks,
            accounted, live, streams: Streams::default(), charge,
        }),
...
fn uncertain_initial_close_retains_registered_outcome_and_resources_through_error_or_unwind() {
    for unwind in [false, true] {
        ...
        let (identity, unit) = registered_preparation(&config, &memory, &mut installed);
        let before = memory.snapshot();
        let root_fd = installed.find(|entry| entry.identity == identity).unwrap().roots().unwrap()
            .values().next().unwrap().file.as_raw_fd();
        let cancel = CensusCancellation::default();
        let run = || {
            let mut registration = PreparingRegistration { installed: &mut installed, identity, committed: false };
            let prepared = registration.prepared();
            let mut session = CensusSession::new(&prepared.roots, &prepared.config, unit, &cancel,
                prepared.accounted.stage(17).unwrap(), &prepared.streams).unwrap();
            assert_eq!(session.advance(), Progress::Pending);
        ...
        let Registration::Preparing(prepared) = &entry.registration else { panic!("lost initial custody"); };
        ...
        let error = NodeDisk::open_fixture(&config, memory.clone(), &CensusCancellation::default()).unwrap_err();
        let crate::DiskOpenError::Failed(error) = error else { panic!("lost retained failure"); };
        assert_eq!(error.downcast_ref::<io::Error>().unwrap().raw_os_error(), Some(libc::EIO));
...
fn ordinary_initial_failure_retires_descriptors_before_admission_credit() {
    let (_root, config, memory) = fixture();
    let mut installed = super::super::registry().lock();
    let (identity, _unit) = registered_preparation(&config, &memory, &mut installed);
    let mut registration = PreparingRegistration { installed: &mut installed, identity, committed: false };
    let prepared = registration.prepared();
    let fds = prepared.roots.values().map(|root| root.file.as_raw_fd())
        .chain(prepared.ancestor_locks.iter().map(AsRawFd::as_raw_fd)).collect::<Vec<_>>();
    ...
    prepared.charge = crate::DiskMemoryLease::new(ObserveRetirement { lease: Some(lease), fds, observed: observed.clone() });
