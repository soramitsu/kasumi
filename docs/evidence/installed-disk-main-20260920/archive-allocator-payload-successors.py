"""Append immutable allocator payload validation evidence; never replace history.
...
Runs162–164 are included only with pinned terminal/drained receipts.
"""
...
INPUTS = SOURCE / 'archive-allocator-payload-successors/inputs.json'
INPUT_SHA = '28da38d91cb6011a88dce69f9292bb532e5e45c37104b839fd6b92e19519bcae'
...
def main():
    ...
    require(len(old) >= 4941, 'Original4941-entry archive missing')
    if len(old) == 4941:
        require(hashlib.sha256(old_bytes).hexdigest() == '03010867d2f05775287ece46fbc3688ec8f231f0a429f853fdf53e81461616fe', 'Original4941 manifest differs')
    verify_archive(old)
    ...
    omission_name = f'allocator-payload-native-binary-omissions/{omission_id}.json'
    ...
    receipt_name = f'allocator-payload-append-receipts/{batch_id}.json'
