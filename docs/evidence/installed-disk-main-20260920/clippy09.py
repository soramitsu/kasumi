...
before=inventory(); (out/'09-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
...
with (out/'09-store-clippy.log').open('wb') as log:
 process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
 print(json.dumps({'running_pid':process.pid,'deadline_seconds':600,'log':str(out/'09-store-clippy.log')}),flush=True)
...
(out/'09-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'09-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
lines=(out/'09-store-clippy.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
raise SystemExit(rc)
