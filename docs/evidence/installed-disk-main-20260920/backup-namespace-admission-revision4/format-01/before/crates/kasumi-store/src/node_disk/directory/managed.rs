...
impl NodeDiskDirectory {
    ...
    pub(crate) fn claim_descendants(
        ...
        if names.is_empty()
            || owner.names.len().checked_add(names.len())
                .is_none_or(|depth| depth >= disk.config.max_depth as usize)
        {
            return Err(io::ErrorKind::InvalidInput.into());
        }
        let root = disk.roots.get(&owner.root).ok_or(io::ErrorKind::InvalidData)?;
        let prefix = owner.names.iter().fold(NamespaceBinding::root(root.identity),
