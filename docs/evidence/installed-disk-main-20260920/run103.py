...
before=inventory(); (out/'103-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
command=['/Users/mtakemiya/.rustup/toolchains/1.97.1-aarch64-apple-darwin/bin/cargo', 'test', '--offline', '--locked', '--no-fail-fast', '-p', 'kasumi-engine', '--all-features', '--test', 'history', '--test', 'lifecycle', '--test', 'schema_activation', '--test', 'staged_transactions', '--', '--test-threads=1', '--nocapture', '--skip', 'encrypted_restart_and_full_restore_preserve_permanent_activation_receipts']
...
def run():
 ...
 with (out/'103-engine-causal-successors.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'103-engine-causal-successors.log')}),flush=True)
 ...
 after=inventory(); result={'scope':'development four integration targets after exact source-identified fixture/accounting corrections; separately reproduced60s schema restore failure excluded and remains required; no final release qualification','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'inventoried_source_unchanged':before==after}
 (out/'103-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'103-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
 lines=(out/'103-engine-causal-successors.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
 raise SystemExit(rc)
...
try:
 ...
 (out/'103-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'103-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
