...
for number in range(152,162):
 ...
 files.update(p for p in out.glob(f'{number}-*') if p.is_file());files.add(out/f'run{number}.py')
files.add(out/'format160.py')
files.update([Path(__file__),out/'prepare-archive-opening-namespace-successors-failed01.py',out/'prepare-archive-opening-namespace-successors-failed01.json'])
