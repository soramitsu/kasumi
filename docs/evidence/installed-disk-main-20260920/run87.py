...
before=inventory(); (out/'87-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
command=['/Users/mtakemiya/.rustup/toolchains/1.97.1-aarch64-apple-darwin/bin/cargo', 'test', '--offline', '--locked', '--no-fail-fast', '-p', 'kasumi-engine', '--all-features', '--test', 'admission', '--test', 'backup_checkpoint', '--test', 'fixture_epoch_clock', '--test', 'retirement', '--test', 'response_release', '--', '--test-threads=1']
...
def run():
 ...
 with (out/'87-engine-integrations.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'87-engine-integrations.log')}),flush=True)
 ...
 (out/'87-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'87-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
 lines=(out/'87-engine-integrations.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
 raise SystemExit(rc)
...
try:
 ...
 (out/'87-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'87-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
