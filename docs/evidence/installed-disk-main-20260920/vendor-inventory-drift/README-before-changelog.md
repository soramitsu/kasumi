...
| `vendor/redb-4.2.0` | 97 | 109 | 24 | 12 | 0 |
| `vendor/openraft-0.9.25` | 600 | 600 | 0 | 0 | 0 |

Only redb differs. Its 24 changed recorded files comprise 23 source/test files plus the updated `KASUMI_PATCH.md`. All 12 unrecorded files are intentional applied release additions: nine still match exact preserved run 161 source bytes; two existing additions and one new payload module match the applied five-file payload patch and run 162 input. Across changed and added source files, 30 match run 161 and five match the applied payload patch. No unexplained file, missing file, symlink, special input, or executable-mode change was found. Run 162 was in progress at audit; matching its input is not a completed-test claim.
...
`report.json` records every expected and observed hash, size, permission and classification, plus review/input hashes. The initial scan and preliminary report are preserved separately; the preliminary report preceded the five-file payload application and contains an outdated note about README wording. This final report supersedes that note.
