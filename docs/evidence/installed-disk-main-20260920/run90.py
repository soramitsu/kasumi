...
before=inventory(); (out/'90-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
command=['/Users/mtakemiya/dev/kasumi/target/debug/deps/kasumi_authority-8a653d8f0b295bfb', '--exact', 'service::tests::coverage_dispatch_precedes_permission_and_survives_exact_encrypted_restart', '--test-threads=1', '--nocapture']
...
def run():
 ...
 with (out/'90-authority-coverage-replay.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'90-authority-coverage-replay.log')}),flush=True)
 ...
 after=inventory(); result={'scope':'development isolated replay of observed authority84 failure using its already-built exact binary; overlaps still-stalled84 and engine89, not final release qualification','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'inventoried_source_unchanged':before==after,'binary_sha256':binary_before,'binary_unchanged':hashlib.sha256(Path(command[0]).read_bytes()).hexdigest()==binary_before}
 (out/'90-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'90-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
 lines=(out/'90-authority-coverage-replay.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
 raise SystemExit(rc)
...
try:
 ...
 (out/'90-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'90-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
