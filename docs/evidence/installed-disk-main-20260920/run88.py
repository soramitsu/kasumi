...
before=inventory(); (out/'88-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
command=['/Users/mtakemiya/.rustup/toolchains/1.97.1-aarch64-apple-darwin/bin/cargo', 'test', '--offline', '--locked', '--no-fail-fast', '-p', 'kasumi-engine', '--all-features', '--test', 'concurrent_pagination', '--test', 'contracts', '--test', 'control', '--test', 'embedded_audit', '--test', 'guarded_staging', '--test', 'history', '--test', 'lifecycle', '--test', 'ordered_seek', '--test', 'replicated', '--test', 'restore_deadline', '--test', 'schema_activation', '--test', 'shutdown', '--test', 'snapshot_budget', '--test', 'staged_transactions', '--', '--test-threads=1', '--nocapture']
...
def run():
 ...
 with (out/'88-engine-integrations.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'88-engine-integrations.log')}),flush=True)
 ...
 after=inventory(); result={'scope':'development migrated engine integration caller regression, original test workloads/deadlines unchanged, not final release qualification','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'inventoried_source_unchanged':before==after}
 (out/'88-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'88-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
 lines=(out/'88-engine-integrations.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
 raise SystemExit(rc)
...
try:
 ...
 (out/'88-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'88-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
