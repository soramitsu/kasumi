...
before=inventory(); (out/'65-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
...
def run():
 ...
 with (out/'65-server-admission-and-cleanup.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'65-server-admission-and-cleanup.log')}),flush=True)
...
try:
 ...
 (out/'65-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'65-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
