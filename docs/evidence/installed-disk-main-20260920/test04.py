...
before=inventory(); (out/'04-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
...
with (out/'04-store-tests.log').open('wb') as log:
 process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
 print(json.dumps({'running_pid':process.pid,'deadline_seconds':900,'log':str(out/'04-store-tests.log')}),flush=True)
...
(out/'04-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'04-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
lines=(out/'04-store-tests.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
raise SystemExit(rc)
