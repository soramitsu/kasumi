...
before=inventory(); (out/'146-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
command=['/Users/mtakemiya/.rustup/toolchains/1.97.1-aarch64-apple-darwin/bin/cargo', 'test', '--offline', '--locked', '--manifest-path', 'vendor/redb-4.2.0/Cargo.toml', '--test', 'canonical_format', 'upstream_two_phase_format_three_is_rejected_before_table_decoding', '--', '--exact', '--test-threads=1', '--nocapture']
...
def run():
 ...
 with (out/'146-canonical-format-diagnostic.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'146-canonical-format-diagnostic.log')}),flush=True)
 ...
 after=inventory(); result={'scope':'Canonical public upstream two-phase format assertion with original error diagnostic; no final acceptance','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'inventoried_source_unchanged':before==after}
 (out/'146-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'146-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
 lines=(out/'146-canonical-format-diagnostic.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
 raise SystemExit(rc)
...
try:
 ...
 (out/'146-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'146-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
