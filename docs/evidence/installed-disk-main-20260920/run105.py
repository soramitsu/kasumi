...
before=inventory(); (out/'105-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
command=['/Users/mtakemiya/.rustup/toolchains/1.97.1-aarch64-apple-darwin/bin/cargo', 'test', '--offline', '--locked', '-p', 'kasumi-authority', '--all-features', '--lib', '--', '--test-threads=1', '--nocapture', '--exact', 'service::tests::issuer_tests::control_epoch_stop_preserves_exact_identity_replay_and_restarts_full_drain', 'service::tests::issuer_tests::target_materialization_tests::activated_target_keeps_operational_suspension_and_membership_across_restart', 'service::tests::issuer_tests::target_materialization_tests::exact_actual_completion_is_required_for_issuer_and_target_activation', 'service::tests::issuer_tests::target_materialization_tests::target_close_diagnostic_rejects_unrelated_storage_failures', 'service::tests::issuer_tests::target_materialization_tests::target_close_replication_diagnostic_preserves_child_failure_boundaries']
...
def run():
 ...
 with (out/'105-authority-original-outcomes.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'105-authority-original-outcomes.log')}),flush=True)
 ...
 after=inventory(); result={'scope':'development three authority failures with original-cause diagnostics and two exact shutdown rejection classifiers; unchanged deadlines/workloads; no release qualification','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'inventoried_source_unchanged':before==after}
 (out/'105-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'105-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
 lines=(out/'105-authority-original-outcomes.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
 raise SystemExit(rc)
...
try:
 ...
 (out/'105-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'105-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
