...
before=inventory(); (out/'164-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
command=['/Users/mtakemiya/.rustup/toolchains/1.97.1-aarch64-apple-darwin/bin/cargo', 'test', '--offline', '--locked', '--manifest-path', 'vendor/redb-4.2.0/Cargo.toml', '--features', 'experimental_cursor', '--no-fail-fast', '--test', 'basic_tests', '--test', 'canonical_format', '--test', 'check_integrity_admitted', '--test', 'check_integrity_failure', '--test', 'corrupted_btree_descent', '--test', 'crash_consistency', '--test', 'cursor_tests', '--test', 'integration_tests', '--test', 'multimap_tests', '--test', 'multithreading_tests', '--', '--test-threads=1', '--nocapture']
...
def run():
 ...
 with (out/'164-vendor-feature-public-integration.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'164-vendor-feature-public-integration.log')}),flush=True)
 ...
 after=inventory(); result={'scope':'All ten vendor public integration targets with experimental cursor/API5 enabled after canonical allocator payload geometry, semantics, shrink and branch routing validation; no final acceptance','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'inventoried_source_unchanged':before==after}
 (out/'164-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'164-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
 lines=(out/'164-vendor-feature-public-integration.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
 raise SystemExit(rc)
...
try:
 ...
 (out/'164-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'164-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
