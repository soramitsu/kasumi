...
manifest={'schema':'kasumi-target-storage-census-adoption-v1','root':'/Users/mtakemiya/dev/kasumi','branch':subprocess.check_output(['git','branch','--show-current'],text=True).strip(),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'files':files,'patch_sha256':sha(r/'adoption.patch'),'apply_check_exit':result.returncode,'target_rustfmt_exit':0,'new_tests':15,'cargo_dispatched':False,'native_tests_dispatched':False,'production_callers_migrated':False,'complete_workspace_bound':False,'readme_sha256':sha(r/'README.md')}
