...
before=inventory(); (out/'96-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
command=['/Users/mtakemiya/.rustup/toolchains/1.97.1-aarch64-apple-darwin/bin/cargo', 'test', '--offline', '--locked', '-p', 'kasumi-authority', '--all-features', '--lib', '--', '--test-threads=1', '--nocapture', '--exact', 'service::tests::coverage_dispatch_precedes_permission_and_survives_exact_encrypted_restart', 'service::tests::encrypted_restart_restarts_full_drain_and_never_reuses_an_old_incarnation', 'service::tests::issuer_tests::control_epoch_stop_preserves_exact_identity_replay_and_restarts_full_drain', 'service::tests::permanent_target_stop_defeats_missing_and_prepared_generations_then_reopens_after_full_drain']
...
def run():
 ...
 with (out/'96-authority-memory-callers.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'96-authority-memory-callers.log')}),flush=True)
 ...
 after=inventory(); result={'scope':'development four authority failures with test-only original unavailable-cause diagnostics; unchanged original workloads/deadlines; not final release qualification','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'inventoried_source_unchanged':before==after}
 (out/'96-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'96-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
 lines=(out/'96-authority-memory-callers.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
 raise SystemExit(rc)
...
try:
 ...
 (out/'96-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'96-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
