...
fn a_new_close_error_is_not_relinquished_by_the_call_that_first_starts_close() {
    ...
    // The new error is returned by the real retained shutdown protocol; the
    // borrowed backend close still releases the actual descriptor successfully.
    opening.registration.owner().state.lock().file.disk().fail();
    ...
    let original = {
        ...
        assert_eq!(redb.settlement(), DatabaseOpenSettlement::Closed);
    ...
    assert_eq!(observed.retire(), StorageCensusDisposition::Retired);
}
