...
before=inventory(); (out/'29-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
...
with (out/'29-raft-custody-presized.log').open('wb') as log:
 process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
 print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'29-raft-custody-presized.log')}),flush=True)
...
after=inventory(); result={'scope':'development integrated raft-custody-presized diagnostic, not final release qualification','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'inventoried_source_unchanged':before==after}
(out/'29-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'29-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
lines=(out/'29-raft-custody-presized.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
raise SystemExit(rc)
