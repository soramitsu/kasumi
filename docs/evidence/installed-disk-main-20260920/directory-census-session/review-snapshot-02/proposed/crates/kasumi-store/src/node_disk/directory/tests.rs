...
fn file_capacity_remains_independent_of_directory_membership() {
    ...
    let old = disk.lock_state().accounted.iter().map(|(key, entry)| (*key, *entry))
        .collect::<std::collections::BTreeMap<_, _>>();
    disk.reconcile(&CensusCancellation::default()).unwrap();
    assert_eq!(disk.lock_state().accounted.iter().map(|(key, entry)| (*key, *entry))
        .collect::<std::collections::BTreeMap<_, _>>(), old);
    assert_eq!(disk.snapshot().phase, NodeDiskPhase::Open);
