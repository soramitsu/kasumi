...
#[derive(Debug)]
pub(super) struct CensusCloseFailure { pub(super) errno: i32 }
impl std::fmt::Display for CensusCloseFailure {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "census stream close also failed: {}", io::Error::from_raw_os_error(self.errno))
    }
}
...
impl<'a> CensusSession<'a> {
    ...
    fn finish(mut self) -> Result<Totals> {
        ...
        match (self.failure.take(), self.streams.close_error()) {
            (Some(error), Some(close)) => Err(error.context(CensusCloseFailure { errno: close.raw_os_error().expect("native close errno") })),
