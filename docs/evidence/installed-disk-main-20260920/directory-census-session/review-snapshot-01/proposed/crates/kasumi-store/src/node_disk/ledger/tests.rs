...
fn retained_and_replacement_bounds_preserve_independent_file_capacity() {
    ...
    for index in 0..n + roots {
        old.insert(Identity(1, index), AccountedInode::Directory(directory(1, None)));
    }
    for index in 0..n {
    ...
    for files in 0..=n {
        for directories in 0..=n {
            ...
            for index in 0..directories + roots {
                new.insert(Identity(4, index), AccountedInode::Directory(directory(4, None)));
            }
            assert_eq!(new.values().filter(|entry| entry.file().is_some()).count() as u64, files);
            assert_eq!(new.values().filter(|entry| entry.directory().is_some()).count() as u64, directories + roots);
...
fn peak_cardinality_overflow_fails_before_allocation() {
    ...
    assert_eq!(capacities(u64::MAX / 2, u64::MAX / 2, 1), Some((u64::MAX, u64::MAX)));
    assert_eq!(capacities(1_000_000, 1_000_000, 1), Some((2_000_001, 2_000_001)));
}
