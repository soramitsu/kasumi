...
impl NodeDiskConfig {
    pub fn validate(&self) -> Result<()> {
        ...
        ensure!(self.max_persistent_files > 0, "persistent file budget is zero");
...
impl NodeDisk {
    ...
    fn open_inner(
        ...
        if let Some(existing) = installed.find(|entry| entry.owner.config.roots == config.roots) {
        ...
        let mut prepared = PreparedCensus {
            resources: Some(PreparedResources {
                roots, ancestor_locks, accounted, live, charge, registry_charge,
            }),
            streams: census::Streams::default(),
        };
        let resources = prepared.resources.as_mut().expect("prepared resources");
        let totals = census(&resources.roots, config, unit, cancel,
            resources.accounted.stage(census_limit)?, &prepared.streams)?;
        cancel.check()?;
        resources.accounted.commit_stage();
        let device_id = resources.roots.values().next().expect("validated roots").identity.0;
        let device = selection.open(device_id, config.min_free_bytes, memory.clone())?;
        let PreparedResources { roots, ancestor_locks, accounted, live, charge, registry_charge } = prepared.take();
        ...
        let entry = List::prepare(RegisteredDisk {
            owner: disk.clone(),
    ...
    pub fn fixture_for_path(
        ...
        let existing = {
            ...
            installed
                .find(|entry| entry.owner.config.binding(path.as_ref()).is_ok())
    ...
    pub fn reconcile(&self, cancel: &CensusCancellation) -> Result<()> {
        ...
        let mut state = self.lock_state();
        if state.phase != NodeDiskPhase::Failed { state.phase = NodeDiskPhase::Paused; }
        ensure!(
            state.open_files == 0
                && state.open_directories == 0
                && state.open_directory_cursors == 0
        ...
        let result = {
            ...
            let State { accounted, census_streams, .. } = &mut *state;
            census(&self.roots, &self.config, self.unit, cancel,
                accounted.stage(census_limit)?, census_streams)
        };
