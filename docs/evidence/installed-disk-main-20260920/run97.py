...
before=inventory(); (out/'97-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
command=['/Users/mtakemiya/.rustup/toolchains/1.97.1-aarch64-apple-darwin/bin/cargo', 'test', '--offline', '--locked', '-p', 'kasumi-authority', '--all-features', '--lib', '--', '--test-threads=1', '--nocapture', '--exact', 'service::tests::issuer_tests::target_materialization_tests::activated_target_keeps_operational_suspension_and_membership_across_restart', 'service::tests::issuer_tests::target_materialization_tests::exact_actual_completion_is_required_for_issuer_and_target_activation', 'service::tests::issuer_tests::target_materialization_tests::expired_completion_with_missing_journal_recovers_only_exact_inspection_facts', 'service::tests::issuer_tests::target_materialization_tests::three_actual_materializations_initialize_and_commit_completion_with_restart_proof', 'service::tests::issuer_tests::target_materialization_tests::target_close_diagnostic_rejects_unrelated_storage_failures']
...
def run():
 ...
 with (out/'97-authority-memory-callers.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'97-authority-memory-callers.log')}),flush=True)
 ...
 after=inventory(); result={'scope':'development four actual target materialization successors plus narrow terminal diagnostic rejection; original deadlines and workload unchanged; not final release qualification','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'inventoried_source_unchanged':before==after}
 (out/'97-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'97-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
 lines=(out/'97-authority-memory-callers.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
 raise SystemExit(rc)
...
try:
 ...
 (out/'97-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'97-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
