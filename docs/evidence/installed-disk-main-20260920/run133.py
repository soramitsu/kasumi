...
before=inventory(); (out/'133-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
command=['/Users/mtakemiya/.rustup/toolchains/1.97.1-aarch64-apple-darwin/bin/cargo', 'test', '--offline', '--locked', '-p', 'kasumi-authority', '--lib', 'issuer_tests::control_epoch_stop_preserves_exact_identity_replay_and_restarts_full_drain', '--', '--test-threads=1', '--nocapture']
...
def run():
 ...
 with (out/'133-stopped-epoch-exact-drain.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'133-stopped-epoch-exact-drain.log')}),flush=True)
 ...
 after=inventory(); result={'scope':'Original stopped-epoch case with exact drain witnesses and same-identity resolution after observed healthy route movement within one original five-second caller budget; no final acceptance','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'inventoried_source_unchanged':before==after}
 (out/'133-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'133-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
 lines=(out/'133-stopped-epoch-exact-drain.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
 raise SystemExit(rc)
...
try:
 ...
 (out/'133-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'133-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
