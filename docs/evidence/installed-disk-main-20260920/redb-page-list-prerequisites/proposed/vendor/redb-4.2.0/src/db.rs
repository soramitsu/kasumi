...
use crate::transactions::{
    ...
    DATA_FREED_TABLE, PageList, PageListKind, SYSTEM_FREED_TABLE, SystemTableDefinition,
...
impl Database {
    ...
    fn check_repaired_allocated_pages_table(
            ...
            .map_err(|e| e.into_storage_error_or_corrupted("Allocated pages table corrupted"))?
        {
            ...
            for result in ReadableTable::iter(&table)? {
                let (_, pages) = result?;
                let pages = pages.value().checked(PageListKind::Data)?;
        ...
        F: FnMut(PageNumber) -> Result,
    {
        ...
        if let Some(definition) = result {
            ...
            for result in ReadableTable::iter(&table)? {
                ...
                let pages = page_list.value().checked(kind)?;
                for i in 0..pages.len() {
                    visitor(pages.get(i))?;
                }
            }
    ...
    fn rebuild_allocator_state(
        ...
        Self::visit_freed_tree(
            ...
            PageListKind::Data,
            mem.clone(),
            |page| mem.mark_page_allocated(page),
        )?;
        Self::visit_freed_tree(
            system_root,
            SYSTEM_FREED_TABLE,
