...
before=inventory(); (out/'98-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
command=['/Users/mtakemiya/.rustup/toolchains/1.97.1-aarch64-apple-darwin/bin/cargo', 'test', '--offline', '--locked', '--no-fail-fast', '-p', 'kasumi-engine', '--all-features', '--test', 'contracts', '--test', 'guarded_staging', '--test', 'schema_activation', '--test', 'staged_transactions', '--', '--test-threads=1', '--nocapture']
...
def run():
 ...
 with (out/'98-engine-fixture-successors.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'98-engine-fixture-successors.log')}),flush=True)
 ...
 after=inventory(); result={'scope':'development four full integration targets after exact contract/private-installation and resident snapshot/terminal point fixture migration; no changed workload or deadline; not final release qualification','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'inventoried_source_unchanged':before==after}
 (out/'98-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'98-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
 lines=(out/'98-engine-fixture-successors.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
 raise SystemExit(rc)
...
try:
 ...
 (out/'98-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'98-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
