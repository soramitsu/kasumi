"""Append immutable storage census and session prerequisite evidence; never replace history.
...
Candidate runs are included only after terminal and drain verification.
"""
...
INPUTS = SOURCE / 'archive-storage-census-sessions/inputs.json'
INPUT_SHA = '0365eff563ca4a69c2d1c61d6200a342194f2abf1cbc6a2eae4a16ca7692dd37'
...
def main():
    ...
    require(len(old) >= 5160, 'Original5160-entry archive missing')
    if len(old) == 5160:
        require(hashlib.sha256(old_bytes).hexdigest() == 'c5920b77dfc114a8047329ca47b3d47829a1d1dc6e355663acd9fd0816f03c68', 'Original5160 manifest differs')
    verify_archive(old)
    ...
    omission_name = f'storage-census-sessions-native-binary-omissions/{omission_id}.json'
    ...
    summary = {'previous_entries_verified': len(old), 'entries_verified': len(old),
        ...
        'terminal_runs_selected': sorted(selected_terminals), 'candidate_terminal_runs': inputs['candidate_terminal_runs']}
    if not additions:
    ...
    receipt_name = f'storage-census-sessions-append-receipts/{batch_id}.json'
