...
//!     let db = Database::create(file.path(), admission.clone())?;
...
pub use db::{
    DatabaseCloseReport, DatabaseCloseSettlement, RetainedDatabase,
    DatabaseOpenMode, DatabaseOpenPhase, DatabaseOpenReport, DatabaseOpenSettlement,
    RetainedDatabaseOpening, OpeningFenceReport,
};
pub use error::{
