...
impl TransactionalMemoryOpening {
    pub(crate) fn new(file: Box<dyn StorageBackend>) -> Self {
        Self { file: Some(file), storage: None }
    }
    pub(crate) fn initialize(
        ...
        assert!(min(region_size, (u64::from(MAX_PAGE_INDEX) + 1) * page_size as u64).is_power_of_two());
        admission.check_owner().map_err(StorageError::from)?;
        self.storage = Some(PagedCachedFile::new_retained(
            &mut self.file, admission, page_size as u64, cache_size,
        )?);
        TransactionalMemory::initialize_retained_storage(
            &mut self.storage, allow_initialize, page_size, requested_region_size,
        )
    }
...
impl TransactionalMemory {
    ...
    pub(crate) fn new(
        ...
        opening.initialize(admission, allow_initialize, page_size, requested_region_size, cache_size)
    }
    ...
    fn initialize_retained_storage(
        ...
        assert!(page_size.is_power_of_two() && page_size >= DB_HEADER_SIZE);
        let region_size = requested_region_size.unwrap_or(MAX_USABLE_REGION_SPACE);
        let region_size = min(region_size, (u64::from(MAX_PAGE_INDEX) + 1) * page_size as u64);
