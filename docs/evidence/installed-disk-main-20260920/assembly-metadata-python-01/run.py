...
EVIDENCE = ROOT / 'target/installed-disk-validation/assembly-metadata-python-01'
sys.path.insert(0, str(ROOT / 'scripts'))
import gate_process
from release_gate import write_json

FILES = (
    'scripts/gate_process.py', 'scripts/release_gate.py',
    'scripts/test_release_gate.py', 'scripts/package_release.py',
    'scripts/test_package_metadata.py', 'scripts/test_package_release.py',
    'scripts/verify_release_acceptance.py', 'scripts/test_verify_release_acceptance.py',
)
...
command = [sys.executable, '-m', 'unittest', '-v', 'test_package_metadata',
           'test_release_gate', 'test_package_release', 'test_verify_release_acceptance']
...
write_json(EVIDENCE / 'invocation.json', {
    ...
    'qualification_scope': 'Python custody and packaging/acceptance regressions only; no native Cargo or repeatable-assembly acceptance',
})
