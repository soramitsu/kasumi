...
r=Path.cwd();p=r/'target/installed-disk-validation/directory-managed-namespace';dependency=r/'target/installed-disk-validation/directory-census-session'
...
manifest={'status':'TARGET_ONLY_UNAPPLIED_SCOPED_NATIVE_PASS_CALLER_MIGRATION_OPEN','branch':subprocess.check_output(['git','branch','--show-current'],text=True).strip(),'root':str(r),'dependency':json.loads((p/'dependency.json').read_text()),'successor':make('successor'),'cumulative':make('cumulative'),'native':{'native-01':'production compile FAIL E0282, no tests; corrected explicit io::Result type in distinct trial','native-02':'43 PASS/4 FAIL: Darwin nlink assumption and fixture absolute ancestor depth; inputs/executables unchanged','native-03':'47 PASS/0 FAIL/5 inherited device tests filtered; all five native phases drained; original120s per phase; 1729 inputs unchanged','results_sha256':digest((p/'native-03/results.json').read_bytes())}}
