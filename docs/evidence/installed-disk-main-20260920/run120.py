...
before=inventory(); (out/'120-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
command=['/usr/bin/lldb', '--batch', '--no-lldbinit', '-o', 'run', '-o', 'process status', '-o', 'thread backtrace all -c 48', '-o', 'process kill', '-o', 'quit', '--', '/Users/mtakemiya/dev/kasumi/target/debug/deps/lifecycle-377b80b89ca111b0', '--test-threads=1', '--nocapture', '--exact', 'recovery_control::recovery_expired_completion_resolves_its_exact_positive_fact_before_activation']
...
def run():
 ...
 with (out/'120-lifecycle-native-stack.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'120-lifecycle-native-stack.log')}),flush=True)
 ...
 after=inventory(); result={'scope':'Diagnostic only: exact117nativebinary underLLDB to capture stack overflow; debuggerexitcode is not a test pass','binary_sha256':'5b01407141bac88d2db20ed91ae5396525f0983b11bd58c0c7d898cbf55615ad','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'inventoried_source_unchanged':before==after}
 (out/'120-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'120-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
 lines=(out/'120-lifecycle-native-stack.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
 raise SystemExit(rc)
...
try:
 ...
 (out/'120-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'120-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
