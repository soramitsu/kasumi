...
before=inventory(); (out/'21-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
command=['/Users/mtakemiya/.rustup/toolchains/1.97.1-aarch64-apple-darwin/bin/cargo', 'test', '--offline', '-p', 'kasumi-engine', '--all-features', '--lib', 'admission::tests::snapshot_startup_inventory_is_charged_without_a_governor_cycle', '--', '--exact', '--test-threads=1']
...
with (out/'21-snapshot-inventory.log').open('wb') as log:
 process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
 print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'21-snapshot-inventory.log')}),flush=True)
...
after=inventory(); result={'scope':'development integrated snapshot-inventory diagnostic, not final release qualification','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'inventoried_source_unchanged':before==after}
(out/'21-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'21-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
lines=(out/'21-snapshot-inventory.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
raise SystemExit(rc)
