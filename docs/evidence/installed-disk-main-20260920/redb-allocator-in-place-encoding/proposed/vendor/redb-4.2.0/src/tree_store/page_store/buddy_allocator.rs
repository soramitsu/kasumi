...
use crate::tree_store::page_store::bitmap::{
    AllocatorEncodingError, BtreeBitmap, check_encoding_buffer, checked_offset_encoded_len,
};
#[cfg(test)]
...
impl BuddyAllocator {
    ...
    /// Encode exactly the current canonical allocator into caller-owned backing.
    ...
    pub(crate) fn to_vec(&self) -> Vec<u8> {
        ...
        let Some(encoded_len) = self.checked_encoded_len() else {
            panic!("Invalid buddy allocator encoding geometry");
        };
        let mut result = vec![0; encoded_len];
        if let Err(error) = self.encode_into(&mut result) {
            panic!("Validated buddy allocator encoding failed: {error:?}");
        }
        result
    }
    ...
    pub(crate) fn reference_to_vec(&self) -> Vec<u8> {
        let serialized: Vec<Vec<u8>> = self
            .free
            .iter()
            .map(BtreeBitmap::reference_to_vec)
            .collect();
    ...
    pub(crate) fn from_bytes(data: &[u8]) -> Self {
        ...
        let mut data_start = FREE_END_OFFSETS + (max_order as usize + 1) * size_of::<u32>();
