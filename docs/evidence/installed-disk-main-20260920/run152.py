...
before=inventory(); (out/'152-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
command=['/Users/mtakemiya/.rustup/toolchains/1.97.1-aarch64-apple-darwin/bin/cargo', 'test', '--offline', '--locked', '-p', 'kasumi-store', '--all-features', '--lib', '--', '--test-threads=1', '--nocapture']
...
def run():
 ...
 with (out/'152-store-canonical-allocator.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'152-store-canonical-allocator.log')}),flush=True)
 ...
 after=inventory(); result={'scope':'Complete kasumi-store library after canonical format4, bounded allocation and DATA reclamation, and one-buffer allocator encoding; no final acceptance','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'inventoried_source_unchanged':before==after}
 (out/'152-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'152-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
 lines=(out/'152-store-canonical-allocator.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
 raise SystemExit(rc)

try:
 run()
except Exception as error:
 recovery={'scope':'Complete kasumi-store library after canonical format4, bounded allocation and DATA reclamation, and one-buffer allocator encoding; no final acceptance', 'error_type':type(error).__name__, 'error':str(error), 'time':time.time(), 'process_inventory':subprocess.check_output(['ps','-axo','pid,ppid,pgid,uid,state,etime,comm'],text=True)}
 (out/'152-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'152-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
