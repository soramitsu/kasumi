...
before=inventory(); (out/'70-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
command=['/Users/mtakemiya/.rustup/toolchains/1.97.1-aarch64-apple-darwin/bin/cargo', 'test', '--offline', '--locked', '-p', 'kasumi-engine', '--all-features', '--test', 'backup_checkpoint', '--', '--test-threads=1']
...
def run():
 ...
 with (out/'70-backup-checkpoint.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'70-backup-checkpoint.log')}),flush=True)
 ...
 after=inventory(); result={'scope':'development full backup checkpoint successor after exact proposal charge and remote fault fixture corrections, not final release qualification','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'inventoried_source_unchanged':before==after}
 (out/'70-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'70-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
 lines=(out/'70-backup-checkpoint.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
 raise SystemExit(rc)
...
try:
 ...
 (out/'70-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'70-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
