...
expected_binary='0f393accc494917f40fb35c4cee06bd88d20532bda6b9ca88eca06a74537dc2f'
def binary_digest():
...
before=inventory(); (out/'137-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
command=['/Users/mtakemiya/.rustup/toolchains/1.97.1-aarch64-apple-darwin/bin/cargo', 'test', '--offline', '--locked', '-p', 'kasumi-engine', '--all-features', '--test', 'lifecycle', 'recovery_control::recovery_uncertain_activation_', '--', '--test-threads=1', '--nocapture']
...
def run():
 ...
 with (out/'137-lifecycle-handoff.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'137-lifecycle-handoff.log')}),flush=True)
 ...
 after=inventory(); result={'scope':'Both original129 failing uncertain-activation cases after current-route fixture and single-write failure diagnostic; unchanged workloads/deadlines, no final acceptance','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'inventoried_source_unchanged':before==after,'binary_path':str(binary),'expected_binary_sha256':expected_binary,'binary_sha256_before':binary_before,'binary_sha256_after':binary_after,'binary_unchanged':binary_before==binary_after}
 (out/'137-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'137-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
 lines=(out/'137-lifecycle-handoff.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
...
try:
 ...
 (out/'137-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'137-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
