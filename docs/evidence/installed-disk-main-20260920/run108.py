...
before=inventory(); (out/'108-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
command=['/Users/mtakemiya/.rustup/toolchains/1.97.1-aarch64-apple-darwin/bin/cargo', 'test', '--offline', '--locked', '-p', 'kasumi-engine', '--all-features', '--lib', '--', '--test-threads=1', '--nocapture', '--skip', 'public_restore_admits_resident_state_without_charging_permanent_stream_as_ram']
...
def run():
 ...
 with (out/'108-engine-library.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'108-engine-library.log')}),flush=True)
 ...
 after=inventory(); result={'scope':'development complete engine library with snapshot-work foundation; original512-by256 public restore remains failed/excluded and100000capacity ignored; no final qualification','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'inventoried_source_unchanged':before==after}
 (out/'108-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'108-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
 lines=(out/'108-engine-library.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
 raise SystemExit(rc)
...
try:
 ...
 (out/'108-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'108-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
