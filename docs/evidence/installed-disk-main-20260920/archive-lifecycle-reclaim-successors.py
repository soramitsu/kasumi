"""Append immutable lifecycle/reclaim evidence; never replace history.
...
Runs135–141 are included only with pinned terminal/drained receipts.
"""
...
INPUTS = SOURCE / 'archive-lifecycle-reclaim-successors/inputs.json'
INPUT_SHA = '0820650c891d1a94c7e0e9cd3ba21700dda5278cc88b4e6b122b9040bb981655'
...
def main():
    ...
    require(len(old) >= 3191, 'Original3191-entry archive missing')
    if len(old) == 3191:
        require(hashlib.sha256(old_bytes).hexdigest() == '16da8612edc5ffd3e478dab8971bbddd156004c1e50a53c17ca4d07e456e7385', 'Original3191 manifest differs')
    verify_archive(old)
    ...
    omission_name = f'lifecycle-reclaim-native-binary-omissions/{omission_id}.json'
    ...
    receipt_name = f'lifecycle-reclaim-append-receipts/{batch_id}.json'
