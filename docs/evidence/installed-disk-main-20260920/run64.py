...
before=inventory(); (out/'64-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
command=['/Users/mtakemiya/.rustup/toolchains/1.97.1-aarch64-apple-darwin/bin/cargo', 'test', '--offline', '--locked', '-p', 'kasumi-engine', '--all-features', '--lib', '--', '--test-threads=1', '--nocapture', 'admission::', 'state::snapshot_bundle::tests::replacement_receives_complete_chain_and_restart_uses_local_dependencies', 'state::tenant_audit::tests::uncertain_publication_keeps_hot_prefix_and_apply_never_recontacts_external_archive']
...
def run():
 ...
 with (out/'64-engine-core-and-archives.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'64-engine-core-and-archives.log')}),flush=True)
 ...
 after=inventory(); result={'scope':'development installed core, exact fixture budget and archive namespace regression diagnostic, not final release qualification','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'inventoried_source_unchanged':before==after}
 (out/'64-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'64-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
 lines=(out/'64-engine-core-and-archives.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
 raise SystemExit(rc)
...
try:
 ...
 (out/'64-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'64-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
