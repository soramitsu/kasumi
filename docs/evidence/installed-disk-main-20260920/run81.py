...
before=inventory(); (out/'81-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
command=['/Users/mtakemiya/.rustup/toolchains/1.97.1-aarch64-apple-darwin/bin/cargo', 'test', '--offline', '--locked', '-p', 'kasumi-engine', '--all-features', '--lib', '--test', 'fixture_epoch_clock', '--test', 'admission', '--', '--test-threads=1', 'admission::', 'admission_startup_tests', 'database_construction_tests', 'physical_fixture_tests', 'snapshot_codec::tests', 'database_worker_outcome_tests', 'public_capture_and_restore_preparation_are_complete_admitted_and_never_publish', 'fixture_epoch_rejects', 'one_epoch_expires_leases_and_credentials_but_preserves_permanent_command_identity', 'reserved_capacity_rejects_proposals_and_queries_but_committed_raft_work_still_applies', 'explicit_local_bootstrap_reads_the_complete_committed_generation']
...
def run():
 ...
 with (out/'81-engine-memory-ownership.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'81-engine-memory-ownership.log')}),flush=True)
 ...
 after=inventory(); result={'scope':'development admitted disk/core/constructor/startup regressions and explicit codec ownership, not final release qualification','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'inventoried_source_unchanged':before==after}
 (out/'81-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'81-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
 lines=(out/'81-engine-memory-ownership.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
 raise SystemExit(rc)
...
try:
 ...
 (out/'81-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'81-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
