"""Append immutable allocator/public integration evidence; never replace history.
...
Runs148–151 are included only with pinned terminal/drained receipts.
"""
...
INPUTS = SOURCE / 'archive-allocator-encoding-successors/inputs.json'
INPUT_SHA = '3523a5a0fb89311637a1bd05817e6226c1d3f29fb4c6f4c49d937c82c891f40f'
...
def main():
    ...
    require(len(old) >= 3979, 'Original3979-entry archive missing')
    if len(old) == 3979:
        require(hashlib.sha256(old_bytes).hexdigest() == 'e0d48fe552dc7cae13be01b81501cf83306381c995c94321006ac72f508b9646', 'Original3979 manifest differs')
    verify_archive(old)
    ...
    omission_name = f'allocator-encoding-native-binary-omissions/{omission_id}.json'
    ...
    receipt_name = f'allocator-encoding-append-receipts/{batch_id}.json'
