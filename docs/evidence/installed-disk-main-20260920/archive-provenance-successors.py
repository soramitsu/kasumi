"""Append immutable current dependency provenance evidence; never replace history.
...
Run165 are included only with pinned terminal/drained receipts.
"""
...
INPUTS = SOURCE / 'archive-provenance-successors/inputs.json'
INPUT_SHA = '3f953010e0fe1f2adb84ad5e7cc72c56cfdccc380deba1e0dd8ba85202e3c0bf'
...
def main():
    ...
    require(len(old) >= 4980, 'Original4980-entry archive missing')
    if len(old) == 4980:
        require(hashlib.sha256(old_bytes).hexdigest() == '2470453a84a8b6f20efeef67eb750c60e3a1cd321f8df7ebd5036db0bb5aa827', 'Original4980 manifest differs')
    verify_archive(old)
    ...
    omission_name = f'provenance-native-binary-omissions/{omission_id}.json'
    ...
    receipt_name = f'provenance-append-receipts/{batch_id}.json'
