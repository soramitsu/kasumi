...
impl<V: Clone> Banks<V> {
    ...
    pub(super) fn try_reserve(&mut self, additional: usize) -> io::Result<()> {
        ...
        // Copy before swap. A clone panic leaves the published active map
        // intact; the partial spare remains owned and will reset on reuse.
        // Production values are Copy inode records or Weak handles, so clone
        // has no application callback and performs no allocation.
        for (key, value) in self.active.iter() {
            self.spare.try_reserve(1)?;
            assert!(self.spare.insert(*key, value.clone()).is_none());
        }
        self.spare.try_reserve(additional)?;
        #[cfg(test)]
        {
            self.rebuilds += 1;
        }
