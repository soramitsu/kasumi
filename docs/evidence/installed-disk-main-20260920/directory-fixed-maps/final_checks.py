...
r=Path.cwd();p=r/'target/installed-disk-validation/directory-fixed-maps';manifest=json.loads((p/'manifest.json').read_text());inputs=[p/'manifest.json',p/'fixed-maps.patch',p/'cumulative.patch']+[f for folder in ('base','proposed','cumulative-base','cumulative-proposed') for f in (p/folder).rglob('*') if f.is_file()]
def sha(f):return hashlib.sha256(f.read_bytes()).hexdigest()
def inventory():return {str(f):sha(f) for f in inputs}
before=inventory();results=[]
for name,baseline,patch in [('successor','base','fixed-maps.patch'),('cumulative','cumulative-base','cumulative.patch')]:shutil.copytree(p/baseline,p/('apply-check-'+name))
formatter=Path('/Users/mtakemiya/.rustup/toolchains/1.97.1-aarch64-apple-darwin/bin/rustfmt')
commands=[('format',[str(formatter),'--check','--edition','2024','--config','skip_children=true',*[str(f) for f in sorted((p/'proposed').rglob('*.rs'))]])]+[(name+'-apply-check',['git','apply','--check','--directory='+str((p/('apply-check-'+name)).relative_to(r)),str(p/patch)]) for name,patch in [('successor','fixed-maps.patch'),('cumulative','cumulative.patch')]]
