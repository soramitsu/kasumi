...
before=inventory(); (out/'53-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
command=['/Users/mtakemiya/.rustup/toolchains/1.97.1-aarch64-apple-darwin/bin/cargo', 'test', '--offline', '--locked', '-p', 'kasumi-engine', '--all-features', '--test', 'admission', '--test', 'fixture_epoch_clock', '--test', 'response_release', '--test', 'backup_checkpoint', '--test', 'retirement', '--', '--test-threads=1']
...
def run():
 ...
 with (out/'53-engine-integration.log').open('wb') as log:
  process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
  print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'53-engine-integration.log')}),flush=True)
 ...
 after=inventory(); result={'scope':'development engine changed admission/backup/retirement integration diagnostic, not final release qualification','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'inventoried_source_unchanged':before==after}
 (out/'53-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'53-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
 lines=(out/'53-engine-integration.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
 raise SystemExit(rc)
...
try:
 ...
 (out/'53-runner-failure.json').write_text(json.dumps(recovery,indent=2)+'\n')
 (out/'53-source-after-recovery.json').write_text(json.dumps(inventory(),indent=2)+'\n')
 raise
