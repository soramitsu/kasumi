...
pub(crate) const TERMINAL_IO_BUFFER_BYTES: usize = terminal::IO_CHUNK_BYTES + 2 * terminal::HEADER_BYTES;
...
impl Directory {
    ...
    fn read_terminal(&self, session: Uuid, slot: Terminal, limit: usize) -> Result<Option<Vec<u8>>> {
        let Some(file) = self.open_leaf(slot.published())? else { return Ok(None); };
    ...
    pub fn put(&self, session_id: Uuid, slot: BackupSessionSlot, bytes: &[u8]) -> Result<()> {
        ...
        let claim = self.1.claim_namespace(claim_root, claim_relative)?;
