...
pub(super) const IO_CHUNK_BYTES: usize = 4096;
...
impl Terminal {
    ...
    pub(super) const fn published(self) -> &'static str {
        match self {
            Self::Intent => "intent.kasumi",
            Self::Outcome => "outcome.kasumi",
        }
    }
    pub(super) const fn reserve(self) -> &'static str {
        match self {
            Self::Intent => "intent.reserve",
            Self::Outcome => "outcome.reserve",
        }
    }
}
...
fn parse_header(bytes: &[u8], session: Uuid, slot: Terminal) -> io::Result<usize> {
    if bytes.len() != HEADER_BYTES || session.is_nil() || &bytes[..8] != MAGIC
    ...
    if length == 0 || length > PAYLOAD_BYTES { return Err(io::ErrorKind::InvalidData.into()); }
    Ok(length)
}

#[cfg(test)]
pub(super) fn decode(bytes: &[u8], session: Uuid, slot: Terminal, limit: usize) -> io::Result<&[u8]> {
    if bytes.len() != FILE_BYTES { return Err(io::ErrorKind::InvalidData.into()); }
    let length = parse_header(&bytes[..HEADER_BYTES], session, slot)?;
    if length > limit { return Err(io::ErrorKind::InvalidInput.into()); }
...
fn read_header(file: &crate::NodeDiskFile, session: Uuid, slot: Terminal)
    -> io::Result<([u8; HEADER_BYTES], usize)> {
    let result = (|| {
        if file.observed_len()? != FILE_BYTES as u64 { return Err(io::ErrorKind::InvalidData.into()); }
        let mut header = [0; HEADER_BYTES];
        file.read_exact_at(&mut header, 0)?;
        let length = parse_header(&header, session, slot)?;
        Ok((header, length))
    })();
    result.inspect_err(|_| file.owner_failed())
}

/// Classification streams one admitted 4KiB buffer. It does not allocate a full
/// terminal payload merely to decide whether the published record exists.
pub(super) fn validate(file: &crate::NodeDiskFile, session: Uuid, slot: Terminal) -> io::Result<()> {
    let (header, length) = read_header(file, session, slot)?;
    let result = (|| {
        let mut hash = Sha256::new();
        let mut buffer = [0; IO_CHUNK_BYTES];
        let mut offset = HEADER_BYTES;
        while offset < FILE_BYTES {
            let count = buffer.len().min(FILE_BYTES - offset);
            file.read_exact_at(&mut buffer[..count], offset as u64)?;
            let body = (HEADER_BYTES + length).saturating_sub(offset).min(count);
            hash.update(&buffer[..body]);
            if buffer[body..count].iter().any(|&byte| byte != 0) { return Err(io::ErrorKind::InvalidData.into()); }
            offset += count;
        }
        if header[40..72] != hash.finalize()[..] { return Err(io::ErrorKind::InvalidData.into()); }
        Ok(())
    })();
    result.inspect_err(|_| file.owner_failed())
}

/// The returned allocation is the exact original ciphertext length, bounded by
/// the caller before allocation. Canonical fixed padding uses the admitted
/// per-file scratch buffer; a small caller limit never allocates a full container.
pub(super) fn read(file: &crate::NodeDiskFile, session: Uuid, slot: Terminal, limit: usize)
    -> io::Result<Vec<u8>> {
    let (header, length) = read_header(file, session, slot)?;
    if length > limit { return Err(io::ErrorKind::InvalidInput.into()); }
    let result = (|| {
        let mut payload = vec![0; length];
        file.read_exact_at(&mut payload, HEADER_BYTES as u64)?;
        if header[40..72] != Sha256::digest(&payload)[..] { return Err(io::ErrorKind::InvalidData.into()); }
        let mut buffer = [0; IO_CHUNK_BYTES];
        let mut offset = HEADER_BYTES + length;
        while offset < FILE_BYTES {
            let count = buffer.len().min(FILE_BYTES - offset);
            file.read_exact_at(&mut buffer[..count], offset as u64)?;
            if buffer[..count].iter().any(|&byte| byte != 0) { return Err(io::ErrorKind::InvalidData.into()); }
            offset += count;
        }
        Ok(payload)
    })();
...
mod tests {
    ...
    fn fixed_terminal_roundtrip_preserves_exact_ciphertext_and_full_extent() {
        ...
        for slot in [Terminal::Intent, Terminal::Outcome] {
            ...
            assert_eq!(
                decode(&bytes, session, slot, PAYLOAD_BYTES).unwrap(),
                payload
            );
