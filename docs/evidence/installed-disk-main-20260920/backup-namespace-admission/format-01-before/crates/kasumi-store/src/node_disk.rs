...
pub(crate) use batch::{MAX_NAMESPACE_PARTS, NamespaceAdmission, NamespacePart, NamespacePartKind};
...
impl NodeDisk {
        ...
        let existing = {
            ...
            installed
                .find(|entry| {
                    entry
                        .config()
                        .is_some_and(|config| config.binding(path.as_ref()).is_ok())
                })
    ...
    pub fn reconcile(&self, cancel: &CensusCancellation) -> Result<()> {
        ...
        if state.phase != NodeDiskPhase::Failed {
            ...
            state.namespace_batch.as_ref().is_none_or(|batch| !batch.witness_live)
                && state.namespace_claim.as_ref().is_none_or(|claim| !claim.witness_live),
