...
impl NodeDiskDirectory {
    ...
    fn create_child_with_admission(
        ...
        let allocation = if let Some(admission) = admission {
            let parent_binding = state.accounted.get(&owner.identity)
                .and_then(AccountedInode::directory).ok_or(io::ErrorKind::InvalidData)?.binding;
...
pub(super) fn open_existing(
    ...
    ready(disk, state)?;
    if names.is_empty() || state.open_directories.checked_add(super::super::batch::reserved_directories(state))
...
fn prepare_child(
    ...
    let disk = &owner.disk;
    ready(disk, state)?;
    if state.open_directories.checked_add(super::super::batch::reserved_directories(state))
        .is_none_or(|n| n >= disk.config.max_open_directories) {
    ...
    if work.is_some()
        && state
            .directories
            .checked_sub(roots)
            .and_then(|n| n.checked_add(u64::from(super::super::batch::reserved_directories(state))))
            .is_none_or(|n| n >= disk.config.max_persistent_subdirectories)
    {
        return Err(io::ErrorKind::StorageFull.into());
    }
    if work.is_some() {
        state.accounted.try_reserve(1 + super::super::batch::reserved_entries(state))?;
    }
