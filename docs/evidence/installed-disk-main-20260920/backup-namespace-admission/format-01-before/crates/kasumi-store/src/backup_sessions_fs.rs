...
impl Directory {
    pub fn open(path: &Path, disk: Arc<crate::NodeDisk>) -> Result<Self> {
        ...
        let directory = disk.open_directory(root, relative.parent().context("directory binding")?)?;
    ...
    pub(crate) fn open_or_create(path: &Path, disk: Arc<crate::NodeDisk>) -> Result<Self> {
        ...
        if relative.as_os_str().is_empty() { return Self::open(path, disk); }
        ...
        let directory = match parent.open_child(&name) {
            ...
            Err(error) if error.kind() == std::io::ErrorKind::NotFound
                ...
                let mut admission = disk.admit_namespace(&[NamespacePart {
                    root, relative, kind: NamespacePartKind::Directory,
    ...
    pub(crate) fn sync_all(&self) -> Result<()> { self.0.sync_all().map_err(Into::into) }
    fn check(&self) -> Result<()> { self.0.sync_all().map_err(Into::into) }
    fn child(&self, name: &str) -> Result<Option<Self>> {
        let name = CString::new(name)?;
        match self.0.open_child(&name) {
            Ok(directory) => Ok(Some(Self(directory, self.1.clone(), self.2.join(name.to_str()?)))),
            Err(error) if error.kind() == std::io::ErrorKind::NotFound
                && self.1.snapshot().phase == crate::NodeDiskPhase::Open => Ok(None),
    ...
    fn session(&self, id: Uuid) -> Result<Option<Self>> {
        ...
        let Some(sessions) = self.child("sessions")? else { return Ok(None); };
    ...
    fn read_terminal(&self, session: Uuid, slot: Terminal, limit: usize) -> Result<Option<Vec<u8>>> {
        let Some(mut bytes) = self.read(slot.published(), terminal::FILE_BYTES)? else { return Ok(None); };
        let length = match terminal::decode(&bytes, session, slot, limit) {
            Ok(payload) => payload.len(),
            Err(error) => { self.1.fail(); return Err(error.into()); }
        };
        bytes.copy_within(terminal::HEADER_BYTES..terminal::HEADER_BYTES + length, 0);
    ...
    fn terminal_state(&self, session: Uuid, slot: Terminal) -> Result<(bool, bool)> {
        let published = self.read_terminal(session, slot, terminal::PAYLOAD_BYTES)?.is_some();
        #[cfg(test)]
    ...
    pub fn put(&self, session_id: Uuid, slot: BackupSessionSlot, bytes: &[u8]) -> Result<()> {
        ...
        // Claim before classification; another wrapper cannot publish between a
        // published-name absence and reserve-name observation and cause a new
        // reserve to be recreated next to an already published terminal.
        let claim_path = self.2.join("sessions").join(session_id.to_string());
        let (claim_root, claim_relative) = self.1.binding(&claim_path)?;
        let _claim = self.1.claim_namespace(claim_root, claim_relative)?;
        ...
        if let Some(terminal) = terminal_slot { terminal::header(session_id, terminal, bytes)?; }
        let sessions = self.child("sessions")?;
        let session = match &sessions { Some(dir) => dir.child(&session_id.to_string())?, None => None };
        let objects = match &session { Some(dir) => dir.child("objects")?, None => None };
        let mut terminals = [(false, false); 2];
        if let Some(session) = &session {
            for (index, terminal) in [Terminal::Intent, Terminal::Outcome].into_iter().enumerate() {
                terminals[index] = session.terminal_state(session_id, terminal)?;
            }
            if terminals.iter().any(|(published, _)| *published)
                && terminals.iter().any(|(published, reserve)| !published && !reserve) {
                self.1.fail();
        ...
        if let Some(terminal) = terminal_slot {
            ...
            if terminals[index].0 { return Err(std::io::Error::from(std::io::ErrorKind::AlreadyExists).into()); }
        }
        ...
        if sessions.is_none() { missing.push((sessions_path, NamespacePartKind::Directory)); }
        if session.is_none() { missing.push((session_path.clone(), NamespacePartKind::Directory)); }
        if objects.is_none() { missing.push((objects_path, NamespacePartKind::Directory)); }
        for (index, terminal) in [Terminal::Intent, Terminal::Outcome].into_iter().enumerate() {
            if terminals[index] == (false, false) {
                missing.push((session_path.join(terminal.reserve()), NamespacePartKind::File { length: terminal::FILE_BYTES as u64 }));
            }
        }
        if terminal_slot.is_none() {
            missing.push((temporary_path.clone(), NamespacePartKind::File { length: bytes.len() as u64 }));
        }
        let mut requests = Vec::with_capacity(missing.len());
        for (path, kind) in &missing {
            let (root, relative) = self.1.binding(path)?;
            requests.push(NamespacePart { root, relative, kind: *kind });
        }
        let mut admission = if requests.is_empty() { None } else {
            Some(self.1.admit_claimed_namespace(&_claim, &requests, crate::DiskWork::Foreground)?)
        };
        let sessions = match sessions {
            Some(dir) => dir,
            None => self.create_child("sessions", admission.as_mut().expect("admitted sessions directory"))?,
        };
        let session = match session {
            Some(dir) => dir,
            None => sessions.create_child(&session_id.to_string(), admission.as_mut().expect("admitted session directory"))?,
        };
        let objects = match objects {
            Some(dir) => dir,
            None => session.create_child("objects", admission.as_mut().expect("admitted objects directory"))?,
        };
        ...
        for (index, terminal) in [Terminal::Intent, Terminal::Outcome].into_iter().enumerate() {
            if terminals[index] == (false, false) {
                ...
                let (root, relative) = self.1.binding(&path)?;
                let file = self.1.create_admitted_file(admission.as_mut().expect("admitted terminal"), root, relative)?;
        ...
        let object = if terminal_slot.is_none() {
            ...
            let file = self.1.create_admitted_file(admission.as_mut().expect("admitted object"), root, relative)?;
            file.grow_reserved(bytes.len() as u64)?;
            file.sync_all_and_parent()?;
            Some(file)
        } else { None };
        if let Some(admission) = admission { admission.finish()?; }
        let (file, destination, name) = if let Some(terminal) = terminal_slot {
            ...
            let file = session.open_leaf(terminal.reserve())?.context("backup terminal reserve absent")?;
            terminal::write(&file, session_id, terminal, bytes)?;
            (file, &session, terminal.published().to_owned())
        } else {
            let BackupSessionSlot::Object(id) = slot else { unreachable!() };
    ...
    pub fn get(
        ...
        match slot {
            ...
            BackupSessionSlot::Outcome => session.read_terminal(session_id, Terminal::Outcome, limit),
    ...
    fn aborted_objects(&self, proof: &VerifiedBackupAbort) -> Result<Option<Self>> {
        ...
        proof.matches_outcome(
            &session
                .read_terminal(
                    proof.session_id(), Terminal::Outcome,
...
pub(crate) mod test_sync {
    ...
    pub(crate) fn pause(directory: &Path, name: &str, point: Point)
        -> Result<(std::sync::mpsc::Receiver<()>, std::sync::mpsc::SyncSender<()>)> {
        let metadata = std::fs::metadata(directory)?;
        let (entered_tx, entered_rx) = std::sync::mpsc::sync_channel(1);
        let (release_tx, release_rx) = std::sync::mpsc::sync_channel(1);
        assert!(pauses().lock().unwrap().insert(
            (metadata.dev(), metadata.ino(), name.to_owned(), point),
            Pause { entered: entered_tx, release: release_rx },
        ).is_none());
    ...
    pub(super) fn check(directory: &Directory, name: &str, point: Point) -> Result<()> {
        ...
        if let Some(pause) = pause {
            pause.entered.send(())?;
            pause.release.recv_timeout(std::time::Duration::from_secs(10))?;
        }
        ensure!(
