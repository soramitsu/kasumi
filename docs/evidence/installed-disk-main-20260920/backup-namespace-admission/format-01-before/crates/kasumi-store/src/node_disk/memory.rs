...
impl NodeDisk {
    pub fn required_metadata_bytes(config: &NodeDiskConfig) -> anyhow::Result<u64> {
        ...
        let root_path = config.roots.values().map(|path| path.as_os_str().as_bytes().len())
            .max().unwrap_or(0);
        ...
        let caller_slot = add(add(caller_path, 256)?, size::<super::batch::NamespacePart<'static>>()?)?;
        let admission_paths = mul(
            super::batch::MAX_NAMESPACE_PARTS as u64,
            add(add(paths, components)?, caller_slot)?,
        )?;
        let base = add(add(FIXED_OWNER_WORKSPACE, admission_paths)?, disk_memory::arc::<Self>()?)?;
        Ok(add(
            base,
            add(
                ledgers,
