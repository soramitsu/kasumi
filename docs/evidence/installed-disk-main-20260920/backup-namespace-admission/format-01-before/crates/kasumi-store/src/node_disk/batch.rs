...
impl BatchRecord {
    fn reserved(&self) -> impl Iterator<Item = &Part> {
        self.parts.iter().flatten().filter(|p| p.progress == Progress::Reserved)
    }
    pub(super) fn reserved_files(&self) -> u32 {
        self.reserved().filter(|p| matches!(p.kind, NamespacePartKind::File { .. })).count() as u32
    }
    pub(super) fn reserved_directories(&self) -> u32 {
        self.reserved().filter(|p| p.kind == NamespacePartKind::Directory).count() as u32
    }
...
impl NamespaceAdmission {
    ...
    fn part_mut<'a>(
        ...
        state.namespace_batch.as_mut()
            .filter(|b| b.id == self.id)
            .and_then(|b| b.parts.iter_mut().flatten().find(|p| p.root == root && p.binding == binding))
    ...
    pub(super) fn work(&self, state: &State) -> io::Result<DiskWork> {
        state.namespace_batch.as_ref().filter(|b| b.id == self.id)
            .map(|b| b.work).ok_or_else(|| io::ErrorKind::InvalidInput.into())
    }
    ...
    pub(crate) fn finish(mut self) -> io::Result<()> {
        ...
        let batch = state.namespace_batch.as_ref().filter(|b| b.id == self.id)
            .ok_or(io::ErrorKind::InvalidInput)?;
        for part in batch.parts.iter().flatten() {
            if part.progress != Progress::Transferred || part.backing.is_some() {
                return Err(io::ErrorKind::InvalidData.into());
            }
            let entry = state.accounted.values().find(|e| e.binding() == part.binding)
            ...
            if !valid { return Err(io::ErrorKind::InvalidData.into()); }
        }
    ...
    pub(crate) fn cancel(mut self) -> io::Result<()> {
        ...
        for part in batch.parts.iter().flatten() {
            if part.progress != Progress::Reserved { return Err(io::ErrorKind::InvalidData.into()); }
            credit = credit.checked_add(part.bytes).ok_or(io::ErrorKind::InvalidData)?;
        }
        let bytes = state.bytes.checked_sub(credit).ok_or(io::ErrorKind::InvalidData)?;
        let pending = state.pending.checked_sub(credit).ok_or(io::ErrorKind::InvalidData)?;
        let mut device = self.disk.device.lock();
        let shared = device.checked_sub(credit).ok_or(io::ErrorKind::InvalidData)?;
        ...
        for part in state.namespace_batch.as_mut().expect("validated batch").parts.iter_mut().flatten() {
            drop(part.backing.take());
            drop(std::mem::take(&mut part.root));
            drop(std::mem::take(&mut part.names));
        }
...
pub(super) fn reserved_files(state: &State) -> u32 {
    state.namespace_batch.as_ref().map_or(0, BatchRecord::reserved_files)
}
pub(super) fn reserved_directories(state: &State) -> u32 {
    state.namespace_batch.as_ref().map_or(0, BatchRecord::reserved_directories)
}
pub(super) fn reserved_entries(state: &State) -> usize {
    state.namespace_batch.as_ref().map_or(0, BatchRecord::reserved_entries)
}
pub(super) fn reserved_binding(state: &State, binding: NamespaceBinding) -> bool {
    state.namespace_batch.as_ref().is_some_and(|b| b.reserved().any(|p| p.binding == binding))
}
pub(super) fn reserved_children(state: &State, parent: NamespaceBinding) -> u64 {
    state.namespace_batch.as_ref().map_or(0, |b| b.reserved_children(parent))
}

/// A single physical namespace mutation lane. Admission is nonblocking: no
/// native writer waits while retaining an unregistered thread or descriptor.
...
impl Drop for NamespaceClaim {
    fn drop(&mut self) {
        let mut state = self.disk.lock_state();
        if state.namespace_claim.as_ref().is_none_or(|claim| claim.id != self.id) { return; }
        if state.phase == NodeDiskPhase::Open && state.namespace_batch.is_none() && state.pending_directory.is_none() {
            // The caller declares this witness before every operation resource,
            // so those resources actually retire before this lane is credited.
            drop(state.namespace_claim.take());
        } else {
            state.namespace_claim.as_mut().expect("matching claim").witness_live = false;
            self.disk.fail_locked(&mut state);
        }
    }
}
...
impl NodeDisk {
    pub(crate) fn claim_namespace(self: &Arc<Self>, root: &str, relative: &Path) -> io::Result<NamespaceClaim> {
        let mut state = self.lock_state();
        if state.phase != NodeDiskPhase::Open || !self.device.lock().admission_ready() {
            return Err(io::ErrorKind::InvalidData.into());
        }
        if state.namespace_claim.is_some() || state.namespace_batch.is_some() || state.pending_directory.is_some() {
            return Err(io::ErrorKind::WouldBlock.into());
        }
        let root = self.roots.get(root).ok_or(io::ErrorKind::InvalidInput)?;
        ...
        let binding = names.iter().fold(NamespaceBinding::root(root.identity), |binding, name| binding.child(name));
        let id = state.namespace_batch_generation.checked_add(1).ok_or(io::ErrorKind::StorageFull)?;
        state.namespace_batch_generation = id;
        state.namespace_claim = Some(ClaimRecord { id, binding, witness_live: true });
        Ok(NamespaceClaim { disk: self.clone(), id })
    }
    ...
    pub(crate) fn admit_claimed_namespace(
        ...
        if !Arc::ptr_eq(self, &claim.disk) { return Err(io::ErrorKind::InvalidInput.into()); }
        self.admit_namespace_inner(requests, work, Some(claim.id))
    }

    fn admit_namespace_inner(
        self: &Arc<Self>, requests: &[NamespacePart<'_>], work: DiskWork, claim: Option<u64>,
    ) -> io::Result<NamespaceAdmission> {
        let mut state = self.lock_state();
        if state.namespace_claim.as_ref().map(|claim| claim.id) != claim {
            return Err(io::ErrorKind::WouldBlock.into());
        }
        if state.namespace_batch.is_some() { return Err(io::ErrorKind::WouldBlock.into()); }
        if state.phase != NodeDiskPhase::Open
            || state.pending_directory.is_some() || requests.is_empty()
            || requests.len() > MAX_NAMESPACE_PARTS {
            return Err(io::ErrorKind::InvalidInput.into());
        }
        let id = state.namespace_batch_generation.checked_add(1).ok_or(io::ErrorKind::StorageFull)?;
        let mut record = BatchRecord { id, work, witness_live: true, parts: std::array::from_fn(|_| None) };
        let mut bytes = 0_u64;
        for (index, request) in requests.iter().enumerate() {
            let root = self.roots.get(request.root).ok_or(io::ErrorKind::InvalidInput)?;
            let names = super::directory::names(request.relative, self.config.max_depth, self.config.max_name_bytes)?;
            if names.is_empty() { return Err(io::ErrorKind::InvalidInput.into()); }
            let mut binding = NamespaceBinding::root(root.identity);
            let mut parent = binding;
            for name in &names { parent = binding; binding = binding.child(name); }
            ...
            match parent_enrolled {
                ...
                None if record.parts.iter().flatten().any(|p| p.binding == parent && p.kind == NamespacePartKind::Directory) => (),
                _ => return Err(io::ErrorKind::InvalidData.into()),
            }
            let count = parent_enrolled.and_then(AccountedInode::directory).map_or(0, |e| e.children)
                .checked_add(record.parts.iter().flatten().filter(|p| p.parent == parent).count() as u64)
                .and_then(|n| n.checked_add(1)).ok_or(io::ErrorKind::StorageFull)?;
            if count > self.config.directory_policy.max_entries { return Err(io::ErrorKind::StorageFull.into()); }
            let (part_bytes, backing) = match request.kind {
                NamespacePartKind::Directory => (self.config.directory_policy.extent_bytes, Backing::Directory(Arc::new_uninit())),
                NamespacePartKind::File { length } => {
                    if length > i64::MAX as u64 { return Err(io::ErrorKind::InvalidInput.into()); }
                    (super::rounded(length, self.unit)?, Backing::File(Arc::new_uninit()))
                }
            };
            bytes = bytes.checked_add(part_bytes).ok_or(io::ErrorKind::StorageFull)?;
            record.parts[index] = Some(Part { root: request.root.to_owned(), names, binding, parent,
                kind: request.kind, bytes: part_bytes, backing: Some(backing), progress: Progress::Reserved });
        }
        let files = record.reserved_files();
        let directories = record.reserved_directories();
        let roots = u64::try_from(self.roots.len()).map_err(|_| io::ErrorKind::InvalidData)?;
        if state.files.checked_add(u64::from(files)).is_none_or(|n| n > self.config.max_persistent_files)
            || state.directories.checked_sub(roots).and_then(|n| n.checked_add(u64::from(directories)))
                .is_none_or(|n| n > self.config.max_persistent_subdirectories)
            || state.open_files.checked_add(files).is_none_or(|n| n > self.config.max_open_files)
            || state.open_directories.checked_add(directories).is_none_or(|n| n > self.config.max_open_directories) {
        ...
        Ok(NamespaceAdmission { disk: self.clone(), id, active: true })
    }
}
