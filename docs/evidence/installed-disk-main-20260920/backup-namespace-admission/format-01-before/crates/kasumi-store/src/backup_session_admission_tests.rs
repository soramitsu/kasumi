...
fn open(path: &Path, files: u64) -> Arc<crate::NodeDisk> {
    ...
    retry_disk_registry(|| crate::NodeDisk::open_fixture(&config, memory.clone(), &crate::CensusCancellation::default())).unwrap()
}
...
fn new_session_denies_before_mkdir_when_both_terminal_inodes_cannot_fit() {
    ...
    assert!(directory.put(Uuid::new_v4(), BackupSessionSlot::Intent, b"intent").is_err());
...
fn terminal_publication_keeps_original_inode_full_extent_and_exact_ciphertext() {
    ...
    directory.put(session, BackupSessionSlot::Intent, b"original encrypted intent").unwrap();
    let path = temporary.path().join("sessions").join(session.to_string());
    assert!(!path.join("intent.reserve").exists());
    assert_eq!(path.join("intent.kasumi").metadata().unwrap().len(), terminal::FILE_BYTES as u64);
    ...
    assert_eq!(directory.get(session, BackupSessionSlot::Intent, 64).unwrap().unwrap(), b"original encrypted intent");
    let charged = disk.snapshot().charged_bytes;
    directory.put(session, BackupSessionSlot::Outcome, b"original encrypted outcome").unwrap();
    assert!(!path.join("outcome.reserve").exists());
    let published = path.join("outcome.kasumi").metadata().unwrap();
    assert_eq!((published.dev(), published.ino()), (reserve.dev(), reserve.ino()));
    assert_eq!(published.len(), terminal::FILE_BYTES as u64);
    assert_eq!(disk.snapshot().charged_bytes, charged);
    assert_eq!(disk.snapshot().persistent_files, 2);
    assert_eq!(directory.get(session, BackupSessionSlot::Outcome, 64).unwrap().unwrap(), b"original encrypted outcome");
}
...
fn two_wrappers_cannot_recreate_reserve_across_terminal_classification_and_rename() {
    ...
    let session = Uuid::new_v4();
    first.put(session, BackupSessionSlot::Intent, b"intent").unwrap();
    let path = temporary.path().join("sessions").join(session.to_string());
    let (entered, release) = test_sync::pause(&path, "outcome.kasumi", test_sync::Point::TerminalClassified).unwrap();
    let worker = std::thread::spawn(move || first.put(session, BackupSessionSlot::Outcome, b"winner"));
    ...
    let competing = second.put(session, BackupSessionSlot::Outcome, b"loser").unwrap_err();
    assert_eq!(competing.downcast_ref::<std::io::Error>().unwrap().kind(), std::io::ErrorKind::WouldBlock);
    assert_eq!(disk.snapshot().phase, crate::NodeDiskPhase::Open);
    assert!(path.join("outcome.reserve").exists());
    ...
    assert!(!path.join("outcome.reserve").exists());
    assert_eq!(second.get(session, BackupSessionSlot::Outcome, 32).unwrap().unwrap(), b"winner");
    assert_eq!(disk.snapshot().persistent_files, 2);
    assert!(second.put(session, BackupSessionSlot::Outcome, b"late").is_err());
    assert!(!path.join("outcome.reserve").exists());
    assert_eq!(disk.snapshot().phase, crate::NodeDiskPhase::Open);
}
...
fn permanent_terminal_capacity_survives_real_process_restart_at_full_file_and_byte_limits() {
    ...
    for stage in ["prepare", "finish"] {
        let output = std::process::Command::new(std::env::current_exe().unwrap())
            .args(["--exact", "backup_sessions::filesystem::session_admission_tests::terminal_restart_worker", "--nocapture", "--test-threads=1"])
...
fn terminal_restart_worker() {
    let Ok(stage) = std::env::var("KASUMI_TERMINAL_RESTART_STAGE") else { return; };
    let path = PathBuf::from(std::env::var_os("KASUMI_TERMINAL_RESTART_ROOT").unwrap());
    let session = Uuid::parse_str(&std::env::var("KASUMI_TERMINAL_RESTART_SESSION").unwrap()).unwrap();
    let mut config = crate::NodeDisk::fixture_config(path.join("unused")).unwrap();
    ...
    let memory = TestDiskMemory::new(256 << 20, 4096);
    let disk = retry_disk_registry(|| crate::NodeDisk::open_fixture(&config, memory.clone(), &crate::CensusCancellation::default())).unwrap();
    let directory = Directory::open(&path, disk.clone()).unwrap();
    if stage == "prepare" {
        directory.put(session, BackupSessionSlot::Intent, b"intent before restart").unwrap();
        let file = disk.create_file("fixture", Path::new("unrelated.kasumi"), crate::DiskWork::Foreground).unwrap();
        let remaining = config.max_bytes - disk.snapshot().charged_bytes;
        file.reserve_growth(0, remaining, crate::DiskWork::Foreground).unwrap();
        file.grow_reserved(remaining).unwrap();
        file.sync_all_and_parent().unwrap();
        drop(file);
        assert_eq!(disk.snapshot().persistent_files, config.max_persistent_files);
        ...
        assert_eq!(disk.snapshot().persistent_files, config.max_persistent_files);
        assert_eq!(disk.snapshot().charged_bytes, config.max_bytes);
        directory.put(session, BackupSessionSlot::Outcome, b"outcome after restart").unwrap();
        assert_eq!(directory.get(session, BackupSessionSlot::Outcome, 64).unwrap().unwrap(), b"outcome after restart");
        assert_eq!(disk.snapshot().persistent_files, config.max_persistent_files);
        assert_eq!(disk.snapshot().charged_bytes, config.max_bytes);
    }
}
