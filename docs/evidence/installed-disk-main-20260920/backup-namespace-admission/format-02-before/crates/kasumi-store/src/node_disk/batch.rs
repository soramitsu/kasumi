...
impl NodeDisk {
    pub(crate) fn claim_namespace(
        ...
        let names =
            super::directory::names(relative, self.config.max_depth, self.config.max_name_bytes)?;
        if names.is_empty() {
            return Err(io::ErrorKind::InvalidInput.into());
        }
        let binding = names
    ...
    fn admit_namespace_inner(
        ...
        let files = requests.iter().filter(|part| matches!(part.kind, NamespacePartKind::File { .. })).count() as u32;
        let directories = requests.iter().filter(|part| part.kind == NamespacePartKind::Directory).count() as u32;
