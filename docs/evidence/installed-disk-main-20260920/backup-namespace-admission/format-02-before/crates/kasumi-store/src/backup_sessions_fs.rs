...
impl Directory {
    ...
    fn read_terminal(
        ...
        let Some(mut bytes) = self.read(slot.published(), terminal::FILE_BYTES)? else {
            return Ok(None);
        };
    ...
    fn terminal_state(&self, session: Uuid, slot: Terminal) -> Result<(bool, bool)> {
        let published = self
            .read_terminal(session, slot, terminal::PAYLOAD_BYTES)?
            .is_some();
        #[cfg(test)]
    ...
    pub fn put(&self, session_id: Uuid, slot: BackupSessionSlot, bytes: &[u8]) -> Result<()> {
        ...
        let mut admission = if requests.is_empty() {
            ...
            Some(
                self.1
                    .admit_claimed_namespace(&_claim, &requests, crate::DiskWork::Foreground)?,
            )
        };
...
mod ownership_tests {
    ...
    fn missing_enrolled_leaf_fences_both_read_and_cleanup_without_releasing_credit() {
        for cleanup in [false, true] {
            ...
            drop(directory);
            disk.reconcile(&crate::CensusCancellation::default()).unwrap();
            assert_eq!(disk.snapshot().charged_bytes, crate::DirectoryPolicy::fixture().extent_bytes);
