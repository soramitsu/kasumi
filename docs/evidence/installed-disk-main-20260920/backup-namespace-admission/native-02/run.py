...
common=[str(compiler),'--edition=2024','--crate-name','backup_namespace_admission_native','-L','dependency='+str(deps),*extern,str(p/'driver.rs')]
commands=[('compiler',[str(compiler),'-vV']),('compile-production',common+['-o',str(p/'production')]),('compile-tests',common+['--test','-o',str(p/'tests')]),('run-production',[str(p/'production')]),('run',[str(p/'tests'),'node_disk::','backup_sessions::','--nocapture','--test-threads=1'])];results=[]
