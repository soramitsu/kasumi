...
impl TableNamespace {
    ...
    #[track_caller]
    fn delete_table(&mut self, dirty: &AtomicBool, name: &str) -> Result<bool, TableError> {
        #[cfg(feature = "logging")]
...
impl WriteTransaction {
    ...
    // commit. The writer guard keeps the DATA root stable; read horizons may
    // advance as readers release. Counting every unselected row makes this
    // conservative in that race. The commit creates no new DATA pending rows.
