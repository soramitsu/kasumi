...
inputs=sorted(list((root/'crates').rglob('*.rs'))+list((root/'crates').rglob('Cargo.toml'))+list((root/'vendor/redb-4.2.0/src').rglob('*.rs'))+list((root/'vendor/openraft-0.9.25').rglob('*.rs'))+[root/'Cargo.toml',root/'Cargo.lock'])
def inventory(): return {str(p.relative_to(root)):hashlib.sha256(p.read_bytes()).hexdigest() for p in inputs}
before=inventory(); (out/'07-source-before.json').write_text(json.dumps(before,indent=2)+'\n')
command=['/Users/mtakemiya/.rustup/toolchains/1.97.1-aarch64-apple-darwin/bin/cargo','check','--offline','--workspace','--all-targets','--all-features']
...
with (out/'07-workspace-check.log').open('wb') as log:
 process=subprocess.Popen(command,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True)
 print(json.dumps({'running_pid':process.pid,'deadline_seconds':1200,'log':str(out/'07-workspace-check.log')}),flush=True)
...
after=inventory(); result={'scope':'development combined workspace compile diagnostic, not final release qualification','command':command,'cwd':str(root),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'process_group':process.pid,'exit_code':rc,'timeout':timeout,'signals':signals,'drained':not live(),'elapsed_seconds':time.time()-started,'store_source_unchanged':before==after}
(out/'07-result.json').write_text(json.dumps(result,indent=2)+'\n'); (out/'07-source-after.json').write_text(json.dumps(after,indent=2)+'\n'); print(json.dumps(result),flush=True)
lines=(out/'07-workspace-check.log').read_text(errors='replace').splitlines(); print('\n'.join(lines[-100:]),flush=True)
raise SystemExit(rc)
