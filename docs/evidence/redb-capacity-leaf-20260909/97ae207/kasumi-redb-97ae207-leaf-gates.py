...
out=pathlib.Path('/tmp/kasumi-redb-97ae207-leaf')
...
before=source();assert before['head']=='97ae2074f7d8a62b60b6550cb36743f48b07581a' and not before['status']
...
record={'scope':'macOS ARM64 seventeen focused redb tests: three allocator candidate regressions plus fourteen growth/admitted-commit regressions. Frozen97ae207 source; prior76f correctness failure and11a compile failure preserved; prior76f failed binary preserved independently. Shared host ordinary correctness cohort, not a quiet performance measurement. No root dependency patch, Kasumi engine/store/server/Raft compilation, listener or provider. Shared host; full upstream just test, fuzzing, durable disk governor and production integration remain unrun.','source_before':before,'tools':{str(p):sha(p) for p in [cargo,rustc,rustdoc]},'runner_sha256':sha(__file__),'runner_interpreter':{'executable':sys.executable,'version':sys.version,'sha256':sha(sys.executable)},'per_gate_timeout_seconds':300,'gates':[]}
