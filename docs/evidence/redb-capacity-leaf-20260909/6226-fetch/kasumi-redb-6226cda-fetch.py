...
out=pathlib.Path('/tmp/kasumi-redb-6226cda-fetch')
...
before=source();assert before['head']=='6226cdaa5b1d18a425a695dbec50b1c4fb2a9760' and not before['status']
...
record={'scope':'macOS ARM64 target-specific locked dependency fetch only, no compilation or tests. No root dependency patch, Kasumi engine/store/server/Raft compilation, listener or provider. Shared host; full upstream just test, fuzzing, durable disk governor and production integration remain unrun.','source_before':before,'tools':{str(p):sha(p) for p in [cargo,rustc,rustdoc]},'runner_sha256':sha(__file__),'per_gate_timeout_seconds':300,'gates':[]}
...
commands=[('locked-leaf-dependencies',[cargo,'fetch','--manifest-path','vendor/redb-4.2.0/Cargo.toml','--locked','--target','aarch64-apple-darwin'],None)]
