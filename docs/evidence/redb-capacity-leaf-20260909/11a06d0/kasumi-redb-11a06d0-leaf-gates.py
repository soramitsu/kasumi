...
out=pathlib.Path('/tmp/kasumi-redb-11a06d0-leaf')
...
before=source();assert before['head']=='11a06d03c65ba6e20701ae194023da9c6f709266' and not before['status']
...
record={'scope':'macOS ARM64 seventeen focused redb tests: three allocator candidate regressions plus fourteen growth/admitted-commit regressions. Frozen11a06d0 source; prior5c2a13PASS binary preserved independently. Shared host ordinary correctness cohort, not a quiet performance measurement. No root dependency patch, Kasumi engine/store/server/Raft compilation, listener or provider. Shared host; full upstream just test, fuzzing, durable disk governor and production integration remain unrun.','source_before':before,'tools':{str(p):sha(p) for p in [cargo,rustc,rustdoc]},'runner_sha256':sha(__file__),'runner_interpreter':{'executable':sys.executable,'version':sys.version,'sha256':sha(sys.executable)},'per_gate_timeout_seconds':300,'gates':[]}
