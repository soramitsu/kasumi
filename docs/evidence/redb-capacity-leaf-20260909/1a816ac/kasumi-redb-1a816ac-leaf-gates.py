...
out=pathlib.Path('/tmp/kasumi-redb-1a816ac-leaf')
...
before=source();assert before['head']=='1a816ac823a1781ad2e335321c878b9af3aabb90' and not before['status']
out.mkdir(exist_ok=False)
prior=pathlib.Path('/tmp/kasumi-redb-76f79ac-leaf/preserved-executables/redb-growth-admission-redb-ba841ece6df4e8b1')
assert sha(prior)=='336b585b2445a29ca0f1cbc7e72f404eab432fb7681056785fa53897769f0f5b'
(out/'prior-failed-76f79ac').mkdir()
shutil.copy2(prior,out/'prior-failed-76f79ac'/prior.name)
assert sha(out/'prior-failed-76f79ac'/prior.name)==sha(prior)
...
record={'scope':'macOS ARM64 seventeen focused redb tests: three allocator candidate regressions plus fourteen growth/admitted-commit regressions. Frozen1a816ac source; prior76f correctness failure and11a/97ae compile failures preserved; prior76f failed binary preserved independently. Shared host ordinary correctness cohort, not a quiet performance measurement. No root dependency patch, Kasumi engine/store/server/Raft compilation, listener or provider. Shared host; full upstream just test, fuzzing, durable disk governor and production integration remain unrun.','source_before':before,'tools':{str(p):sha(p) for p in [cargo,rustc,rustdoc]},'runner_sha256':sha(__file__),'runner_interpreter':{'executable':sys.executable,'version':sys.version,'sha256':sha(sys.executable)},'per_gate_timeout_seconds':300,'gates':[]}
