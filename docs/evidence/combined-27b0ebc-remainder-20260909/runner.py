...
def main():
    ...
    for path in (root, target):
        ...
        locks.append(handle)
    module = importlib.util.spec_from_file_location('frozen_gate_process', root / 'scripts/gate_process.py')
    ...
    record = {'scope': plan['scope'], 'status': 'running', 'source': before['head'],
              ...
              'lock_sha256': before['lock_sha256'], 'runner_sha256': digest(__file__), 'process_helper_sha256': digest(root / 'scripts/gate_process.py'),
