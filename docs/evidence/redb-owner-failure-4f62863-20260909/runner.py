...
def main():
    ...
    for path in (root, target):
        ...
        locks.append(handle)
    helper = Path(plan['process_helper']).resolve()
    if digest(helper) != plan['process_helper_sha256']:
        raise RuntimeError('process helper does not match pinned input')
    module = importlib.util.spec_from_file_location('frozen_gate_process', helper)
    ...
    record = {'scope': plan['scope'], 'status': 'running', 'source': before['head'],
              ...
              'lock_sha256': before['lock_sha256'], 'runner_sha256': digest(__file__), 'process_helper': str(helper), 'process_helper_sha256': digest(helper),
    ...
    try:
        for gate in record['gates']:
            ...
            if digest(helper) != plan['process_helper_sha256']:
                raise RuntimeError('process helper changed before dispatch')
            ...
            def observe(value):
                gate['process_record'] = value
                gate['process_group'] = value['process_group']
                atomic(evidence, record)
            with log.open('wb') as stream:
                outcome = custody.run(command, root, environment, stream, gate['timeout_seconds'], observe)
            code, error = outcome['exit_code'], outcome['error']
            cleanup = dict(outcome['cleanup'], remaining_after_command=bool(outcome['cleanup']['before']))
            if not cleanup['drained'] or cleanup['errors']:
                raise RuntimeError('gate custody is uncertain; output remains unverified')
            gate.update(duration_seconds=round(time.monotonic() - started, 3),
                        process_exit_code=outcome['process_exit_code'], error=error, cleanup=cleanup,
        ...
        record['process_helper_unchanged'] = digest(helper) == plan['process_helper_sha256']
        if not record['source_unchanged'] or not record['process_helper_unchanged']:
            record['status'] = 'failed'
        atomic(evidence, record)
