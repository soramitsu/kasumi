...
lines = [
    ...
    'At 1,000 samples, p99 has roughly ten tail observations. There is one selected '
    'observation per case, with no confidence interval or saturation claim. Mixed-workload percentiles '
...
lines += table(['Deployment', 'Tenants', 'Configured resident data voters', 'Control groups', 'Service audit stores', 'Qualified observation'],
...
lines += ['Each tenant has one logical data Raft group; replicated rows count its three resident voter instances. '
          'Service audit stores share a writer per node and are counted separately from '
