#!/usr/bin/env python3
"""Repeat only network measurements after the TLS-only correction; preserve all standard resource and identity guards.
...
ROOT = next(parent for parent in Path(__file__).resolve().parents if (parent / 'scripts/run_benchmark_matrix.py').is_file())
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="kasumi-matrix-") as temporary:
                ...
                for count in tenants:
                    commands = [(f"{mode}-{count}", [str(executables[0]),"--documents",str(options.documents),"--operations",str(options.operations),"--tenants",str(count),"--modes",mode,"--output",str(output/f"{mode}-{count}.json"),"--work-parent",temporary]) for mode in ()]
